...
)
{
  ...
  glsl_to_spirv_desc.code_size = desc->code_size;
...
)
{
  ...
  for ( compiled_shaders_count = 0; compiled_shaders_count < creation->stages_count; ++compiled_shaders_count )
  {
    ...
    if ( !creation->compiled )
    {
      ...
      desc.code_size = stage->code_size;
    ...
    else
    {
      ...
      spirv_optimized_absolute_filepath = crude_string_buffer_append_use_f(
        ...
        gpu->environment->directories.compiled_shaders_absolute_directory,
...
)
{
  ...
  if ( creation->profiler.enabled )
  {
    ...
    crude_gfx_rhi_set_query_pool_debug_name( &gpu->rhi_device, cmd_pool->profiler.rhi_timestamp_query_pool, "timestamp_query_pool" );
    ...
    crude_gfx_rhi_set_query_pool_debug_name( &gpu->rhi_device, cmd_pool->profiler.rhi_pipeline_stats_query_pool, "pipeline_stats_query_pool" );
  }
#endif /* CRUDE_GFX_GPU_PROFILER */
