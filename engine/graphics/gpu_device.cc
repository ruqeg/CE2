...
)
{
  ...
  
  {
    ...
    gpu->gpu_time_queries_per_frame = 64;
    gpu->num_threads = 1u;
    ...
    
    {
      ...
      crude_string_raw_copy( cmd_buffer_creation.name, "immediate_transfer_cmd_buffer", sizeof( cmd_buffer_creation.name ) );
  ...
  for ( uint32 i = 0; i < CRUDE_GFX_SWAPCHAIN_IMAGES_MAX_COUNT; ++i )
  {
    ...
    crude_gfx_rhi_set_semaphore_debug_name( &gpu->rhi_device, gpu->rhi_image_avalivable_semaphores[ i ], CRUDE_STRING_VIEW_MAKE( "image_avalivable_semaphores" ) );
    crude_gfx_rhi_set_semaphore_debug_name( &gpu->rhi_device, gpu->rhi_swapchain_updated_semaphore[ i ], CRUDE_STRING_VIEW_MAKE( "swapchain_updated_semaphore" ) );
    crude_gfx_rhi_set_semaphore_debug_name( &gpu->rhi_device, gpu->rhi_rendering_finished_semaphore[ i ], CRUDE_STRING_VIEW_MAKE( "rendering_finished_semaphore" ) );
  }
  
  crude_gfx_rhi_create_fence( &gpu->rhi_device, true, &gpu->rhi_immediate_fence );
  crude_gfx_rhi_set_fence_debug_name( &gpu->rhi_device, gpu->rhi_immediate_fence, CRUDE_STRING_VIEW_MAKE( "immediate_fence" ) );
  
  crude_gfx_rhi_create_semaphore( &gpu->rhi_device, true, &gpu->rhi_graphics_semaphore );
  crude_gfx_rhi_set_semaphore_debug_name( &gpu->rhi_device, gpu->rhi_graphics_semaphore, CRUDE_STRING_VIEW_MAKE( "graphics_semaphore" ) );
  ...
  
  {
    ...
    default_sampler_creation.name           = CRUDE_STRING_VIEW_MAKE( "sampler default" );
  ...
  if ( crude_gfx_rhi_get_type( ) == CRUDE_GFX_RHI_TYPE_VULKAN )
  {
    ...
    gpu->blas_identity_transform_hga = crude_gfx_memory_allocate_with_pname( gpu, sizeof( vk_transform_matrix ), CRUDE_GFX_MEMORY_TYPE_CPU_GPU, "blas_identity_transform_hga", CRUDE_GFX_RHI_BUFFER_USAGE_ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY_BIT_KHR );
    crude_memory_copy( gpu->blas_identity_transform_hga.cpu_address, &vk_transform_matrix, sizeof( vk_transform_matrix ) );
  }
  else if ( crude_gfx_rhi_get_type( ) == CRUDE_GFX_RHI_TYPE_NULL )
  {
    gpu->blas_identity_transform_hga = crude_gfx_memory_allocate_with_pname( gpu, sizeof( XMFLOAT4X4 ), CRUDE_GFX_MEMORY_TYPE_CPU_GPU, "blas_identity_transform_hga", CRUDE_GFX_RHI_BUFFER_USAGE_ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY_BIT_KHR );
  }
  else
  {
    CRUDE_ASSERT( false );
...
)
{
  ...
  if ( crude_gfx_memory_allocation_valid( &gpu->blas_identity_transform_hga ) )
  {
    crude_gfx_memory_deallocate( gpu, gpu->blas_identity_transform_hga );
  }
...
)
{
  ...
  
  {
    crude_gfx_rhi_semaphore_submit_info wait_semaphores[] = {
      { gpu->rhi_graphics_semaphore, gpu->absolute_frame, CRUDE_GFX_RHI_PIPELINE_STAGE_TOP_OF_PIPE_BIT, 0 }
    };
 ...
 
  {
    ...
    crude_gfx_cmd_add_image_barrier_ext2( cmd, gpu->rhi_swapchain_images[ gpu->swapchain_image_index ], gpu->rhi_swapchain_images_states[ gpu->swapchain_image_index ], CRUDE_GFX_RHI_RESOURCE_STATE_COPY_DEST, 0, 1, false );
    ...
    gpu->rhi_swapchain_images_states[ gpu->swapchain_image_index ] = CRUDE_GFX_RHI_RESOURCE_STATE_PRESENT;
  ...
  
    {
      crude_gfx_rhi_semaphore_submit_info wait_semaphores[] = {
        { gpu->rhi_rendering_finished_semaphore[ gpu->swapchain_image_index ], 0, CRUDE_GFX_RHI_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, 0 },
        { gpu->rhi_image_avalivable_semaphores[ gpu->current_frame ], 0, CRUDE_GFX_RHI_PIPELINE_STAGE_ALL_COMMANDS_BIT, 0 },
      };
...
(
  _In_ crude_gfx_device                                   *gpu,
  _In_ crude_gfx_compile_shader_description               *desc,
...
)
{
  ...
  glsl_to_spirv_desc.code = desc->code;
  glsl_to_spirv_desc.stage = desc->stage;
  glsl_to_spirv_desc.pass_name = desc->name;
  glsl_to_spirv_desc.define_name = desc->define_name;
  glsl_to_spirv_desc.define_flags = desc->define_flags;
  glsl_to_spirv_desc.temporary_absolute_directory = gpu->environment->temporary_absolute_directory;
  glsl_to_spirv_desc.compiled_absolute_directory = gpu->environment->compiled_shaders_absolute_directory;
  glsl_to_spirv_desc.optimized = desc->optimized;
...
)
{
  ...
  crude_string_raw_copy( texture_creation.name, texture->name, sizeof( texture_creation.name ) );
...
(
  _In_ crude_gfx_device                                   *gpu,
  _In_ crude_string_view const                             technique_name
)
{
...
crude_gfx_access_technique_pipeline_by_name
(
  _In_ crude_gfx_device                                   *gpu,
  _In_ crude_string_view const                             technique_name,
  _In_ uint32                                              flags
)
{
  ...
  return technique->pipelines[ flags ];
}
...
)
{
  ...
  if ( texture )
  {
    ...
    {
      texture->rhi_image.vk.vma_allocation = NULL;
    }
...
)
{
  ...
  for ( compiled_shaders_count = 0; compiled_shaders_count < creation->stages_count; ++compiled_shaders_count )
  {
    ...
    {
      crude_gfx_compile_shader_description                 desc;
      char                                                *spirv_absolute_filepath;

      desc = CRUDE_COMPOUNT_EMPTY( crude_gfx_compile_shader_description );
      desc.code = stage->code;
      desc.define_flags = creation->define_flags;
      desc.define_name = creation->define_name;
      desc.name = creation->name;
      desc.optimized = creation->optimized;
      desc.stage = stage->type;
      crude_gfx_compile_shader( gpu, &desc, gpu->allocator, &spirv_absolute_filepath );
      
      code = NULL;
      code_size = 0;
      
      crude_read_file_binary( crude_string_view_make( spirv_absolute_filepath ), NULL, &code_size );
      
      code = CRUDE_CAST( uint32*, crude_heap_allocator_allocate( gpu->allocator, code_size ) );
      
      crude_read_file_binary( crude_string_view_make( spirv_absolute_filepath ), CRUDE_CAST( uint8*, code ), &code_size );
    ...
    else
    {
      ...
      crude_string_view                                    spirv_optimized_absolute_filepath;
      crude_string_view                                    shader_name;
      crude_string_view                                    compiler_extension;
    
      shader_name = crude_string_view_valid( creation->name ) ? creation->name : CRUDE_STRING_VIEW_MAKE( "unknown" );
      compiler_extension = crude_gfx_rhi_shader_stage_to_compiler_extension( stage->type );

      crude_string_buffer_initialize(
        &absolute_filepath_string_buffer,
        gpu->environment->compiled_shaders_absolute_directory.length + shader_name.length + compiler_extension.length + 64,
        crude_heap_allocator_pack( gpu->allocator ) );
    
      spirv_optimized_absolute_filepath = crude_string_buffer_append_use_f(
        &absolute_filepath_string_buffer,
        "%s\\%s.%s.shader_opt.spv",
        gpu->environment->compiled_shaders_absolute_directory.data,
        shader_name.data,
        compiler_extension.data );
    ...
    CRUDE_ASSERTM( CRUDE_CHANNEL_GRAPHICS, rhi_creation_info.code && rhi_creation_info.code_size, "\"%s\" shader code contains an error or empty!", crude_string_view_valid( creation->name ) ? creation->name.data : "unkown" );
...
)
{
  ...
  crude_string_raw_copy( pipeline->name, creation->name.data, sizeof( pipeline->name ) );
  ...
  else if ( shader_state->pipeline_type == CRUDE_GFX_PIPELINE_TYPE_RAY_TRACING )
  {
    ...
    shader_binding_table_creation.type_flags = CRUDE_GFX_RHI_BUFFER_USAGE_SHADER_BINDING_TABLE_BIT_KHR | CRUDE_GFX_RHI_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT;
    ...
    CRUDE_STRING_RAW_COPY( shader_binding_table_creation.name, "shader_binding_table_raygen" );
    ...
    CRUDE_STRING_RAW_COPY( shader_binding_table_creation.name, "shader_binding_table_hit" );
    ...
    CRUDE_STRING_RAW_COPY( shader_binding_table_creation.name, "shader_binding_table_miss" );
  ...
  crude_gfx_rhi_set_pipeline_debug_name( &gpu->rhi_device, pipeline->rhi_pipeline, crude_string_view_make( pipeline->name ) );
...
)
{
  ...
  crude_string_raw_copy( buffer->name, creation->name, sizeof( buffer->name ) );
  ...
  crude_gfx_rhi_set_buffer_debug_name(  &gpu->rhi_device, buffer->rhi_buffer, crude_string_view_make( creation->name ) );
...
)
{
  ...
  crude_string_raw_copy( framebuffer->name, creation->name, sizeof( framebuffer->name ) );
...
)
{
  ...
  if ( creation->profiler.enabled )
  {
    ...
    crude_gfx_rhi_set_query_pool_debug_name( &gpu->rhi_device, cmd_pool->profiler.rhi_timestamp_query_pool, CRUDE_STRING_VIEW_MAKE( "timestamp_query_pool" ) );
    ...
    rhi_statistics_pool_info.pipeline_statistics = CRUDE_GFX_RHI_QUERY_PIPELINE_STATISTIC_FRAGMENT_SHADER_INVOCATIONS_BIT | CRUDE_GFX_RHI_QUERY_PIPELINE_STATISTIC_COMPUTE_SHADER_INVOCATIONS_BIT;
    if ( gpu->mesh_shaders_extension_present )
    {
      rhi_statistics_pool_info.pipeline_statistics |= CRUDE_GFX_RHI_QUERY_PIPELINE_STATISTIC_MESH_SHADER_INVOCATIONS_BIT_EXT;
      rhi_statistics_pool_info.pipeline_statistics |= CRUDE_GFX_RHI_QUERY_PIPELINE_STATISTIC_TASK_SHADER_INVOCATIONS_BIT_EXT;
    }
    else
    {
      rhi_statistics_pool_info.pipeline_statistics |= CRUDE_GFX_RHI_QUERY_PIPELINE_STATISTIC_CLIPPING_INVOCATIONS_BIT;
      rhi_statistics_pool_info.pipeline_statistics |= CRUDE_GFX_RHI_QUERY_PIPELINE_STATISTIC_CLIPPING_PRIMITIVES_BIT;
      rhi_statistics_pool_info.pipeline_statistics |= CRUDE_GFX_RHI_QUERY_PIPELINE_STATISTIC_INPUT_ASSEMBLY_VERTICES_BIT;
      rhi_statistics_pool_info.pipeline_statistics |= CRUDE_GFX_RHI_QUERY_PIPELINE_STATISTIC_INPUT_ASSEMBLY_PRIMITIVES_BIT;
      rhi_statistics_pool_info.pipeline_statistics |= CRUDE_GFX_RHI_QUERY_PIPELINE_STATISTIC_VERTEX_SHADER_INVOCATIONS_BIT;
    }
    crude_gfx_rhi_create_query_pool( &gpu->rhi_device, &rhi_statistics_pool_info, &cmd_pool->profiler.rhi_pipeline_stats_query_pool );
    crude_gfx_rhi_set_query_pool_debug_name( &gpu->rhi_device, cmd_pool->profiler.rhi_pipeline_stats_query_pool, CRUDE_STRING_VIEW_MAKE( "pipeline_stats_query_pool" ) );
  }
#endif /* CRUDE_GFX_GPU_PROFILER */
...
)
{
  ...
  *cmd_buffer = CRUDE_COMPOUNT_EMPTY( crude_gfx_cmd_buffer );
  ...
  crude_string_raw_copy( cmd_buffer->name, creation->name, sizeof( cmd_buffer->name ) );
  ...
  crude_gfx_rhi_set_command_buffer_debug_name( &gpu->rhi_device, cmd_buffer->rhi_cmd_buffer, crude_string_view_make( creation->name ) );
...
)
{
  ...
  crude_string_raw_copy( technique->name, creation->name, sizeof( technique->name ) );
  technique->handle = technique_handle;

  CRUDE_ARRAY_INITIALIZE_WITH_LENGTH( technique->pipelines, creation->pipelines_count, crude_heap_allocator_pack( gpu->allocator ) );
  for ( size_t i = 0; i < creation->pipelines_count; ++i )
  {
    technique->pipelines[ i ] = creation->pipelines[ i ];
  }
  
  if ( creation->name )
  {
    CRUDE_HASHMAPSTR_SET( gpu->resource_cache.techniques, crude_string_view_make( technique->name ), technique );
  }
...
)
{
  ...
  for ( size_t i = 0; i < CRUDE_ARRAY_LENGTH( technique->pipelines ); ++i )
  {
    crude_gfx_destroy_pipeline( gpu, technique->pipelines[ i ] );
  }
  CRUDE_ARRAY_DEINITIALIZE( technique->pipelines );

  CRUDE_HASHMAPSTR_REMOVE( gpu->resource_cache.techniques, crude_string_view_make( technique->name ) );
  crude_gfx_release_technique( gpu, technique->handle );
}
...
{
  return CRUDE_CAST( crude_gfx_sampler*, crude_resource_pool_access_resource( &gpu->samplers, handle.index ) );
}
...
{
  return CRUDE_CAST( crude_gfx_texture*, crude_resource_pool_access_resource( &gpu->textures, handle.index ) );
}
...
{
  return CRUDE_CAST( crude_gfx_render_pass*, crude_resource_pool_access_resource( &gpu->render_passes, handle.index ) );
}
...
{
  return CRUDE_CAST( crude_gfx_shader_state*, crude_resource_pool_access_resource( &gpu->shaders, handle.index ) );
}
...
{
  return CRUDE_CAST( crude_gfx_pipeline*, crude_resource_pool_access_resource( &gpu->pipelines, handle.index ) );
}
...
{
  return CRUDE_CAST( crude_gfx_buffer*, crude_resource_pool_access_resource( &gpu->buffers, handle.index ) );
}
...
{
  return CRUDE_CAST( crude_gfx_descriptor_set_layout*, crude_resource_pool_access_resource( &gpu->descriptor_set_layouts, handle.index ) );
}
...
{
  return CRUDE_CAST( crude_gfx_descriptor_set*, crude_resource_pool_access_resource( &gpu->descriptor_sets, handle.index ) );
}
...
{
  return CRUDE_CAST( crude_gfx_framebuffer*, crude_resource_pool_access_resource( &gpu->framebuffers, handle.index ) );
}
...
{
  return CRUDE_CAST( crude_gfx_cmd_pool*, crude_resource_pool_access_resource( &gpu->cmd_pools, handle.index ) );
}
...
{
  return CRUDE_CAST( crude_gfx_cmd_buffer*, crude_resource_pool_access_resource( &gpu->cmd_buffers, handle.index ) );
}
...
{
  return CRUDE_CAST( crude_gfx_technique*, crude_resource_pool_access_resource( &gpu->techniques, handle.index ) );
}
...
)
{
  ...
  {
    gpu->rhi_swapchain_images_states[ i ] = CRUDE_GFX_RHI_RESOURCE_STATE_UNDEFINED;
  }
}
...
)
{
  ...
  crude_gfx_rhi_set_descriptor_pool_debug_name( &gpu->rhi_device, gpu->rhi_bindless_descriptor_pool, CRUDE_STRING_VIEW_MAKE( "rhi_bindless_descriptor_pool" ) );
  
  crude_gfx_rhi_create_descriptor_pool( &gpu->rhi_device, false, &gpu->rhi_descriptor_pool );
  crude_gfx_rhi_set_descriptor_pool_debug_name( &gpu->rhi_device, gpu->rhi_descriptor_pool, CRUDE_STRING_VIEW_MAKE( "rhi_descriptor_pool" ) );
  ...
  ds_creation.name = CRUDE_STRING_VIEW_MAKE( "bindless_descriptor_set" );
...
)
{
  ...
  crude_string_raw_copy( texture->name, creation->name, sizeof( texture->name ) );
  ...
  if ( CRUDE_RESOURCE_HANDLE_IS_INVALID( texture->alias_texture_handle ) )
  {
    ...
    crude_gfx_rhi_set_image_allocation_debug_name( &gpu->rhi_device, texture->rhi_image, crude_string_view_make( creation->name ) );
  }
  else
  {
  ...
  crude_gfx_rhi_set_image_debug_name( &gpu->rhi_device, texture->rhi_image, crude_string_view_make( creation->name ) );
  ...
  view_creation.name = crude_string_view_make( texture->name );
