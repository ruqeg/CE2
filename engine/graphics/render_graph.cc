...
(
  _In_ crude_gfx_render_graph                             *render_graph,
  _In_ crude_string_view const                             render_graph_relative_filepath,
...
)
{
  ...
  CRUDE_LOG_INFO( CRUDE_CHANNEL_GRAPHICS, "Parse render graph \"%s\"", render_graph_relative_filepath.data );

  /* Parse */
  {
    uint8                                                   *render_graph_json_buffer;
    crude_string_view                                        render_graph_absolute_filepath;
    crude_string_buffer                                      render_graph_absolute_filepath_string_buffer;
    uint32                                                   render_graph_json_buffer_size;

    crude_string_buffer_initialize( &render_graph_absolute_filepath_string_buffer, environment->render_graph_absolute_directory.length + render_graph_relative_filepath.length + 1, crude_heap_allocator_pack( render_graph->builder->allocator ) );
    render_graph_absolute_filepath = crude_string_buffer_append_use_f( &render_graph_absolute_filepath_string_buffer, "%s%s", environment->render_graph_absolute_directory.data, render_graph_relative_filepath.data );
    
    crude_read_file( render_graph_absolute_filepath, crude_heap_allocator_pack( render_graph->builder->allocator ), &render_graph_json_buffer, &render_graph_json_buffer_size );
    if ( render_graph_json_buffer )
    {
      render_graph_json = cJSON_ParseWithLength( CRUDE_CAST( char const*, render_graph_json_buffer ), render_graph_json_buffer_size );
    }
    else
    {
      CRUDE_LOG_ERROR( CRUDE_CHANNEL_GRAPHICS, "Cannot parse a file \"%s\" to parse render graph", render_graph_absolute_filepath );
    }
    
    crude_string_buffer_deinitialize( &render_graph_absolute_filepath_string_buffer );
    crude_heap_allocator_deallocate( render_graph->builder->allocator, render_graph_json_buffer );
  }
  
  if ( !render_graph_json )
  {
    CRUDE_LOG_ERROR( CRUDE_CHANNEL_GRAPHICS, "Cannot parse a file \"%s\" for render graph... Error %s", render_graph_relative_filepath, cJSON_GetErrorPtr() );
    return;
  }
  ...
  cJSON_ArrayForEach( pass, passes )
  {
    ...
    crude_string_buffer_initialize( &temporary_string_buffer, CRUDE_RKILO( 4 ), crude_heap_allocator_pack( render_graph->builder->allocator ) );
    ...
    CRUDE_ARRAY_INITIALIZE_WITH_CAPACITY( node_creation.inputs, cJSON_GetArraySize( pass_inputs ), crude_heap_allocator_pack( render_graph->builder->allocator ) );
    CRUDE_ARRAY_INITIALIZE_WITH_CAPACITY( node_creation.outputs, cJSON_GetArraySize( pass_outputs ), crude_heap_allocator_pack( render_graph->builder->allocator ) );

    pass_multisample_enabled = cJSON_GetObjectItemCaseSensitive( pass, "multisample" );
    node_creation.multisampled = pass_multisample_enabled ? cJSON_GetNumberValue( pass_multisample_enabled ) : 1;
    ...
    {
      if ( crude_string_raw_cmp( cJSON_GetStringValue( pass_pipeline_type ), "compute" ) == 0 )
      {
        node_creation.type = CRUDE_GFX_RENDER_GRAPH_NODE_TYPE_COMPUTE;
      }
      else if ( crude_string_raw_cmp( cJSON_GetStringValue( pass_pipeline_type ), "ray_tracing" ) == 0 )
      {
    ...
    CRUDE_ARRAY_DEINITIALIZE( node_creation.inputs );
    CRUDE_ARRAY_DEINITIALIZE( node_creation.outputs );
...
{
  CRUDE_LOG_INFO( CRUDE_CHANNEL_GRAPHICS, "Compile render graph" );
  ...
  for ( uint32 node_index = 0; node_index < CRUDE_ARRAY_LENGTH( render_graph->nodes ); ++node_index )
  {
    ...
    for ( uint32 input_index = 0; input_index < CRUDE_ARRAY_LENGTH( node->inputs ); ++input_index )
    {
      ...
      output_resource = crude_gfx_render_graph_builder_access_resource_by_name( render_graph->builder, crude_string_view_make( resource->name ) );
  ...
  /* Topological sorting */
  {
    ...
    CRUDE_ARRAY_INITIALIZE_WITH_CAPACITY( sorted_nodes, CRUDE_ARRAY_LENGTH( render_graph->nodes ), crude_heap_allocator_pack( render_graph->builder->allocator ) );
    CRUDE_ARRAY_INITIALIZE_WITH_LENGTH( visited, CRUDE_ARRAY_LENGTH( render_graph->nodes ), crude_heap_allocator_pack( render_graph->builder->allocator ) );
    CRUDE_ARRAY_INITIALIZE_WITH_CAPACITY( stack, CRUDE_ARRAY_LENGTH( render_graph->nodes ), crude_heap_allocator_pack( render_graph->builder->allocator ) );
  ...
  /* Compute Resource Aliasing */
  {
    ...
    CRUDE_ARRAY_INITIALIZE_WITH_LENGTH( allocations, resource_count, crude_heap_allocator_pack( render_graph->builder->allocator ) );
    CRUDE_ARRAY_INITIALIZE_WITH_LENGTH( deallocations, resource_count, crude_heap_allocator_pack( render_graph->builder->allocator ) );
    CRUDE_ARRAY_INITIALIZE_WITH_CAPACITY( free_list, resource_count, crude_heap_allocator_pack( render_graph->builder->allocator ) );
      ...
      for ( uint32 output_index = 0; output_index < CRUDE_ARRAY_LENGTH( node->outputs ); ++output_index )
      {
        ...
        if ( !output_resource->resource_info.external && CRUDE_RESOURCE_HANDLE_IS_INVALID( allocations[ output_resource_index ] ) )
        {
          ...
          if ( output_resource->type == CRUDE_GFX_RENDER_GRAPH_RESOURCE_TYPE_ATTACHMENT )
          {
            ...
            crude_string_raw_copy( output_resource_texture_creation.name, output_resource->name, sizeof( output_resource_texture_creation.name ) );
            ...
            // !TODO output_resource_texture_creation.alias = ( CRUDE_ARRAY_LENGTH( free_list ) > 0 ) ? CRUDE_ARRAY_POP( free_list ) : CRUDE_GFX_TEXTURE_HANDLE_INVALID;
  ...
  for ( uint32 node_index = 0; node_index < CRUDE_ARRAY_LENGTH( render_graph->nodes ); ++node_index )
  {
    ...
    if ( ( node->type != CRUDE_GFX_RENDER_GRAPH_NODE_TYPE_COMPUTE ) && CRUDE_RESOURCE_HANDLE_IS_INVALID( node->render_pass ) )
    {
      ...
      for ( uint32 output_index = 0; output_index < CRUDE_ARRAY_LENGTH( node->outputs ); ++output_index )
      {
        ...
        if ( output_resource->type == CRUDE_GFX_RENDER_GRAPH_RESOURCE_TYPE_ATTACHMENT )
        {
          if ( crude_gfx_rhi_format_has_depth_or_stencil( info->texture.format ) )
          {
    ...
    if ( CRUDE_RESOURCE_HANDLE_IS_INVALID( node->framebuffer ) )
    {
      ...
      crude_string_raw_copy( framebuffer_creation.name, node->name, sizeof( framebuffer_creation.name ) );
      ...
      for ( uint32 output_index = 0; output_index < CRUDE_ARRAY_LENGTH( node->outputs ); ++output_index )
      {
        ...
        {
          CRUDE_ASSERT( height == CRUDE_CAST( int32, render_graph->builder->gpu->renderer_size.y * output_resource_info->texture.scale.y ) );
        }

        if ( crude_gfx_rhi_format_has_depth_or_stencil( output_resource_info->texture.format ) )
        {
      ...
      for ( uint32 input_index = 0; input_index < CRUDE_ARRAY_LENGTH( node->inputs ); ++input_index )
      {
        ...
        crude_gfx_render_graph_resource *resource = crude_gfx_render_graph_builder_access_resource_by_name( render_graph->builder, crude_string_view_make( input_resource->name ) );
...
)
{
  for ( uint32 node_index = 0; node_index < CRUDE_ARRAY_LENGTH( render_graph->nodes ); ++node_index )
  {
    ...
    crude_gfx_cmd_push_marker( gpu_commands, crude_string_view_make( node->name ) );
    ...
    if ( node->type == CRUDE_GFX_RENDER_GRAPH_NODE_TYPE_GRAPHICS )
    {
      ...
      scissor.x = 0; 
      scissor.y = 0;
      scissor.width = CRUDE_CAST( uint16, width );
      scissor.height = CRUDE_CAST( uint16, height );
      ...
      dev_viewport.rect.width = CRUDE_CAST( uint16, width );
      dev_viewport.rect.height = CRUDE_CAST( uint16, height );
...
{
  builder->allocator = gpu->allocator;
  builder->gpu = gpu;
...
(
  _In_ crude_gfx_render_graph_builder                     *builder,
  _In_ crude_string_view const                             name,
  _In_ crude_gfx_render_graph_pass_container               render_pass
)
{
  int64 handle_index = CRUDE_HASHMAPSTR_GET_INDEX( builder->render_pass_cache.render_pass_map, name );
  if ( handle_index >= 0 )
  {
    CRUDE_ASSERT( false );
    return;
  ...
  handle_index = CRUDE_HASHMAPSTR_GET_INDEX( builder->node_cache.node_map, name );
  if ( handle_index < 0 )
  {
    CRUDE_ASSERT( false );
    return;
  ...
  CRUDE_HASHMAPSTR_SET( builder->render_pass_cache.render_pass_map, name, render_pass );
...
{
  CRUDE_HASHMAPSTR_REMOVE( builder->render_pass_cache.render_pass_map, name );
}
...
{
  for ( uint32 i = 0; i < CRUDE_HASHMAPSTR_CAPACITY( builder->render_pass_cache.render_pass_map ); ++i )
  {
    if ( crude_hashmapstr_backet_key_hash_valid( builder->render_pass_cache.render_pass_map[ i ].key.key_hash ) )
    {
      builder->render_pass_cache.render_pass_map[ i ].key.key_hash = CRUDE_HASHMAPSTR_BACKET_STATE_EMPTY;
    }
  }
}
...
)
{
  ...
  crude_string_raw_copy( node->name, creation->name.data, sizeof( node->name ) );
  node->enabled = creation->enabled;
  node->type = creation->type;
  node->multisampled = creation->multisampled;
  node->framebuffer = CRUDE_GFX_FRAMEBUFFER_HANDLE_INVALID;
  node->render_pass = CRUDE_GFX_RENDER_PASS_HANDLE_INVALID;
  CRUDE_ARRAY_INITIALIZE_WITH_LENGTH( node->inputs, CRUDE_ARRAY_LENGTH( creation->inputs ), crude_heap_allocator_pack( builder->allocator ) );
  CRUDE_ARRAY_INITIALIZE_WITH_LENGTH( node->outputs, CRUDE_ARRAY_LENGTH( creation->outputs ), crude_heap_allocator_pack( builder->allocator ) );
  CRUDE_ARRAY_INITIALIZE_WITH_LENGTH( node->edges, CRUDE_ARRAY_LENGTH( creation->outputs ), crude_heap_allocator_pack( builder->allocator ) );
  
  CRUDE_HASHMAPSTR_SET( builder->node_cache.node_map, crude_string_view_make( node->name ), node_handle );
...
)
{
  ...
  crude_string_raw_copy( resource->name, creation->name.data, sizeof( resource->name ) );
  ...
  if ( creation->type != CRUDE_GFX_RENDER_GRAPH_RESOURCE_TYPE_REFERENCE )
  {
    ...
    CRUDE_HASHMAPSTR_SET( builder->resource_cache.resource_map, crude_string_view_make( resource->name ), resource_handle );
  }
...
{
  return CRUDE_CAST( crude_gfx_render_graph_node*, crude_resource_pool_access_resource( &builder->node_cache.nodes, handle.index ) );
}
...
{
  int64 handle_index = CRUDE_HASHMAPSTR_GET_INDEX( builder->node_cache.node_map, name );
  if ( handle_index < 0 )
  {
    return NULL;
  }
  return CRUDE_CAST( crude_gfx_render_graph_node*, crude_resource_pool_access_resource( &builder->node_cache.nodes, builder->node_cache.node_map[ handle_index ].value.index ) );
...
{
  return CRUDE_CAST( crude_gfx_render_graph_resource*, crude_resource_pool_access_resource( &builder->resource_cache.resources, handle.index ) );
}
...
{
  int64 handle_index = CRUDE_HASHMAPSTR_GET_INDEX( builder->resource_cache.resource_map, name );
  if ( handle_index < 0 )
  {
    CRUDE_ASSERT( false );
    return NULL;
  ...
  return CRUDE_CAST( crude_gfx_render_graph_resource*, crude_resource_pool_access_resource( &builder->resource_cache.resources, builder->resource_cache.resource_map[ handle_index ].value.index ) );
}
...
{
  crude_resource_pool_initialize( &builder->resource_cache.resources, crude_heap_allocator_pack( builder->allocator ), CRUDE_GFX_RENDER_GRAPH_RESOURCES_MAX_COUNT, sizeof( crude_gfx_render_graph_resource ) );
  builder->resource_cache.resource_map = NULL;
  CRUDE_HASHMAPSTR_INITIALIZE( builder->resource_cache.resource_map, crude_heap_allocator_pack( builder->allocator ) );
}
...
{
  crude_resource_pool_initialize( &builder->node_cache.nodes, crude_heap_allocator_pack( builder->allocator ), CRUDE_GFX_RENDER_GRAPH_NODES_MAX_COUNT, sizeof( crude_gfx_render_graph_node ) );
  builder->node_cache.node_map = NULL;
  CRUDE_HASHMAPSTR_INITIALIZE( builder->node_cache.node_map, crude_heap_allocator_pack( builder->allocator ) );
}
...
{
  CRUDE_HASHMAPSTR_INITIALIZE( builder->render_pass_cache.render_pass_map, crude_heap_allocator_pack( builder->allocator ) );
}
...
{
  for ( int32 i = 0; i < CRUDE_HASHMAPSTR_CAPACITY( builder->resource_cache.resource_map ); ++i )
  {
    if ( !crude_hashmapstr_backet_key_hash_valid( builder->resource_cache.resource_map[ i ].key.key_hash ) )
    {
      continue;
    }
  ...
  CRUDE_HASHMAPSTR_DEINITIALIZE( builder->resource_cache.resource_map );
}
...
)
{
  ...
  CRUDE_HASHMAPSTR_DEINITIALIZE( builder->node_cache.node_map );
}
...
{
  CRUDE_HASHMAPSTR_DEINITIALIZE( builder->render_pass_cache.render_pass_map );
}
