...
)
{
  ...
  CRUDE_STRING_RAW_COPY( texture_creation.name, "imgui_font" );
  ...
  pass->vertex_hga = crude_gfx_memory_allocate_with_pname( gpu, vertex_buffer_size_, CRUDE_GFX_MEMORY_TYPE_GPU, "imgui_vertex_hga", 0 );
  pass->index_hga = crude_gfx_memory_allocate_with_pname( gpu, index_buffer_size_, CRUDE_GFX_MEMORY_TYPE_GPU, "imgui_index_hga", 0 );
}
...
)
{
  ...
  pass = CRUDE_CAST( crude_gfx_imgui_pass*, ctx );
  ...
  for ( int32 n = 0; n < draw_counts; ++n )
  {
    ...
    for ( int32 cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; ++cmd_i )
    {
      ...
      if ( texture_handle.index != pass->font_texture.index )
      {
        ...
        //crude_gfx_cmd_add_image_barrier( primary_cmd, texture, CRUDE_GFX_RHI_RESOURCE_STATE_PIXEL_SHADER_RESOURCE, 0u, 1u, crude_gfx_rhi_format_has_depth( texture->vk_format ) );
      }
    }
  }
...
)
{
  ...
  imgui_pipeline = CRUDE_GFX_ACCESS_TECHNIQUE_PIPELINE_BY_NAME( "imgui" );
