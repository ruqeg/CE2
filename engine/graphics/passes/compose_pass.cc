#include <engine/core/profiler.h>
#include <engine/core/hashmapstr.h>
#include <engine/scene/scene_ecs.h>
#include <engine/graphics/scene_renderer.h>

#define COMPOSE_LIGHT
#include <engine/graphics/shaders/compose.crude_shader>

#include <engine/graphics/passes/compose_pass.h>

void
crude_gfx_compose_pass_initialize
(
  _In_ crude_gfx_compose_pass                             *pass,
  _In_ crude_gfx_scene_renderer                           *scene_renderer
)
{
  pass->scene_renderer = scene_renderer;
}

void
crude_gfx_compose_pass_deinitialize
(
  _In_ crude_gfx_compose_pass                             *pass
)
{
}

void
crude_gfx_compose_pass_render
(
  _In_ void                                               *ctx,
  _In_ crude_gfx_cmd_buffer                               *primary_cmd
)
{
  crude_gfx_compose_pass                                  *pass;
  crude_gfx_device                                        *gpu;
  crude_gfx_pipeline_handle                                pipeline;
  crude_gfx_compose_light_pass_push_constant_              pust_constant;
  uint32                                                   flags;

  pass = CRUDE_CAST( crude_gfx_compose_pass*, ctx );
  gpu = pass->scene_renderer->gpu;

  flags = 0u;
  if ( pass->scene_renderer->world_environment.ssr.type != CRUDE_WORLD_ENVIRONMENT_SSR_TYPE_NONE )
  {
    flags |= COMPOSE_LIGHT_FLAG_SSR;
  }
  if ( pass->scene_renderer->ddgi_enabled )
  {
    flags |= COMPOSE_LIGHT_FLAG_INDIRECT;
  }

  pipeline = CRUDE_GFX_ACCESS_TECHNIQUE_PIPELINE_BY_NAME_WITH_FLAGS( "compose_light", flags );
  crude_gfx_cmd_bind_pipeline( primary_cmd, pipeline );
  
  pust_constant = CRUDE_COMPOUNT_EMPTY( crude_gfx_compose_light_pass_push_constant_ );
  pust_constant.direct_radiance_texture_index = CRUDE_GFX_PASS_TEXTURE_INDEX( compose_pass.direct_radiance_texture );
  pust_constant.indirect_radiance_texture_index = CRUDE_GFX_PASS_TEXTURE_INDEX( compose_pass.indirect_radiance_texture );
  pust_constant.radiance_texture_index = CRUDE_GFX_PASS_TEXTURE_INDEX( compose_pass.radiance_texture );
  pust_constant.albedo_texture_index = CRUDE_GFX_PASS_TEXTURE_INDEX( compose_pass.albedo_texture );
  pust_constant.ssr_radiance_texture_index = CRUDE_GFX_PASS_TEXTURE_INDEX( compose_pass.ssr_texture );
  pust_constant.roughness_metalness_texture_index = CRUDE_GFX_PASS_TEXTURE_INDEX( compose_pass.roughness_metalness_texture );
  pust_constant.inv_resolution.x = 1.f / pass->scene_renderer->gpu->renderer_size.x;
  pust_constant.inv_resolution.y = 1.f / pass->scene_renderer->gpu->renderer_size.y;
  crude_gfx_cmd_push_constant( primary_cmd, &pust_constant, sizeof( pust_constant ) );

  crude_gfx_cmd_bind_bindless_descriptor_set( primary_cmd );

  crude_gfx_cmd_dispatch( primary_cmd, ( pass->scene_renderer->gpu->renderer_size.x + 7 ) / 8, ( pass->scene_renderer->gpu->renderer_size.y + 7 ) / 8, 1u );
}

crude_gfx_render_graph_pass_container
crude_gfx_compose_pass_pack
(
  _In_ crude_gfx_compose_pass                             *pass
)
{
  crude_gfx_render_graph_pass_container container = crude_gfx_render_graph_pass_container_empty();
  container.ctx = pass;
  container.render = crude_gfx_compose_pass_render;
  return container;
}