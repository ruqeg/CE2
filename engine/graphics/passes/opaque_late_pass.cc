...
)
{
  ...
  pass = CRUDE_CAST( crude_gfx_opaque_late_pass*, ctx );
...
)
{
  ...
  pipeline = gpu->mesh_shaders_extension_present 
    ? crude_gfx_access_technique_pipeline_by_name( gpu, "opaque_meshlet", 0u )
    : crude_gfx_access_technique_pipeline_by_name( gpu, "opaque_classic", 0u );
...
)
{
  ...
  container.ctx = pass;
  container.render = crude_gfx_opaque_late_pass_render;
  container.pre_render = crude_gfx_opaque_late_pass_pre_render;
  return container;
}