...
)
{
  ...
  if ( times >= 0 )
  {
    ...
    pipeline = crude_gfx_access_technique_pass_by_name( gpu, "volumetric_fog", "noise_baking" )->pipeline;
  ...
  /* Compute Inject Data */
  {
    ...
    pipeline = crude_gfx_access_technique_pass_by_name( gpu, "volumetric_fog", "compute_inject_data" )->pipeline;
    crude_gfx_cmd_bind_pipeline( primary_cmd, pipeline );
