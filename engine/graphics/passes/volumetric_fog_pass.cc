...
)
{
  ...
  CRUDE_STRING_RAW_COPY( texture_creation.name, "froxel_data_texture" );
  ...
  texture_creation.type = CRUDE_GFX_TEXTURE_TYPE_TEXTURE_3D;
  texture_creation.flags = CRUDE_GFX_TEXTURE_MASK_COMPUTE;
  CRUDE_STRING_RAW_COPY( texture_creation.name, "volumetric_noise" );
  ...
  sampler_creation.name = CRUDE_STRING_VIEW_MAKE( "volumetric_tiling_sampler" );
...
)
{
  ...
  pass = CRUDE_CAST( crude_gfx_volumetric_fog_pass*, ctx );
  ...
  if ( times >= 0 )
  {
    ...
    pipeline = CRUDE_GFX_ACCESS_TECHNIQUE_PIPELINE_BY_NAME( "noise_baking" );
  ...
  /* Compute Inject Data */
  {
    ...
    pipeline = CRUDE_GFX_ACCESS_TECHNIQUE_PIPELINE_BY_NAME( "compute_inject_data" );
    crude_gfx_cmd_bind_pipeline( primary_cmd, pipeline );
