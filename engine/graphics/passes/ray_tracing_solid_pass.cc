...
)
{
  ...
  ray_tracing_solid_texture_handle = crude_gfx_render_graph_builder_access_resource_by_name( pass->scene_renderer->render_graph->builder, CRUDE_STRING_VIEW_MAKE( "ray_tracing_solid" ) )->resource_info.texture.handle;
  ray_tracing_solid_texture = crude_gfx_access_texture( gpu, ray_tracing_solid_texture_handle );

  pipeline = CRUDE_GFX_ACCESS_TECHNIQUE_PIPELINE_BY_NAME( "ray_tracing_solid" );
  ...
  push_constant.sbt_stride = pass->scene_renderer->gpu->ray_tracing_pipeline_properties.shader_group_handle_alignment;
