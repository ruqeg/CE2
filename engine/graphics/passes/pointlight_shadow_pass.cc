...
)
{
  ...
  pass->enabled = true;
  pass->culled_pointlight_spheres_hga = crude_gfx_memory_allocate_with_pname( scene_renderer->gpu, sizeof( XMUINT4 ) * CRUDE_LIGHTS_MAX_COUNT, CRUDE_GFX_MEMORY_TYPE_GPU, "culled_pointlight_spheres_hga", 0 );
  pass->culled_pointshadow_meshlet_draw_commands_hga = crude_gfx_memory_allocate_with_pname( scene_renderer->gpu, sizeof( XMUINT4 ) * ( CRUDE_LIGHTS_MAX_COUNT * 4u ), CRUDE_GFX_MEMORY_TYPE_GPU, "culled_pointshadow_meshlet_draw_commands_hga", 0 );
  pass->culled_pointshadow_meshletes_instances_hga = crude_gfx_memory_allocate_with_pname( scene_renderer->gpu, sizeof( XMUINT2 ) * CRUDE_MAX_MESHLETS_PER_LIGHT * CRUDE_LIGHTS_MAX_COUNT, CRUDE_GFX_MEMORY_TYPE_GPU, "culled_pointshadow_meshletes_instances_hga", 0 );
  pass->culled_pointshadow_meshletes_instances_count_hga = crude_gfx_memory_allocate_with_pname( scene_renderer->gpu, sizeof( uint32 ) * ( CRUDE_LIGHTS_MAX_COUNT + 1u ), CRUDE_GFX_MEMORY_TYPE_GPU, "culled_pointshadow_meshletes_instances_count_hga", 0 );
  ...
  crude_string_copy( texture_creation.name, "culled_tetrahedron_shadow_texture", sizeof( texture_creation.name ) );
  pass->culled_tetrahedron_shadow_texture = crude_gfx_create_texture( scene_renderer->gpu, &texture_creation );

  framebuffer_creation = crude_gfx_framebuffer_creation_empty( );
  framebuffer_creation.depth_stencil_texture = pass->culled_tetrahedron_shadow_texture;
  crude_string_copy( framebuffer_creation.name, "culled_tetrahedron_framebuffer", sizeof( framebuffer_creation.name ) );
  ...
  render_pass_creation.name = "culled_tetrahedron_render_pass";
  ...
  sampler_creation.name = "culled_tetrahedron_shadow_sampler";

  pass->tetrahedron_shadow_sampler = crude_gfx_create_sampler( scene_renderer->gpu, &sampler_creation );
  
  crude_gfx_link_texture_sampler( scene_renderer->gpu, pass->culled_tetrahedron_shadow_texture, pass->tetrahedron_shadow_sampler );
}
...
)
{
  ...
  crude_gfx_memory_deallocate( gpu, pass->culled_pointlight_spheres_hga );
  crude_gfx_memory_deallocate( gpu, pass->culled_pointshadow_meshlet_draw_commands_hga );
  crude_gfx_memory_deallocate( gpu, pass->culled_pointshadow_meshletes_instances_hga );
  crude_gfx_memory_deallocate( gpu, pass->culled_pointshadow_meshletes_instances_count_hga );

  crude_gfx_destroy_sampler( gpu, pass->tetrahedron_shadow_sampler );
  crude_gfx_destroy_texture( gpu, pass->culled_tetrahedron_shadow_texture );
...
)
{
  ...
  pass = CRUDE_CAST( crude_gfx_pointlight_shadow_pass*, ctx );
  gpu = pass->scene_renderer->gpu;
  ...
  pointshadow_culling_pipeline = crude_gfx_access_technique_pipeline_by_name( pass->scene_renderer->gpu, "pointshadow_culling", 0u );
  pointshadow_commands_generation_pipeline = crude_gfx_access_technique_pipeline_by_name( pass->scene_renderer->gpu, "pointshadow_commands_generation", 0u );
  pointshadow_pipeline = crude_gfx_access_technique_pipeline_by_name( pass->scene_renderer->gpu, "pointshadow", 0u );
  ...
  /* Pointshadow Culling */
  {
    ...
    crude_gfx_cmd_add_buffer_barrier( primary_cmd, pass->culled_pointshadow_meshletes_instances_hga.buffer_handle, CRUDE_GFX_RHI_RESOURCE_STATE_SHADER_RESOURCE, CRUDE_GFX_RHI_RESOURCE_STATE_UNORDERED_ACCESS );
    crude_gfx_cmd_add_buffer_barrier( primary_cmd, pass->culled_pointshadow_meshletes_instances_count_hga.buffer_handle, CRUDE_GFX_RHI_RESOURCE_STATE_INDIRECT_ARGUMENT, CRUDE_GFX_RHI_RESOURCE_STATE_UNORDERED_ACCESS );
  
    crude_gfx_cmd_fill_buffer( primary_cmd, pass->culled_pointshadow_meshletes_instances_count_hga.buffer_handle, 0u );
    ...
    push_constant.culled_pointshadow_meshletes_instances = pass->culled_pointshadow_meshletes_instances_hga.gpu_address;
    push_constant.culled_pointshadow_meshletes_instances_count = pass->culled_pointshadow_meshletes_instances_count_hga.gpu_address;
    ...
    crude_gfx_cmd_add_buffer_barrier( primary_cmd, pass->culled_pointshadow_meshletes_instances_count_hga.buffer_handle, CRUDE_GFX_RHI_RESOURCE_STATE_UNORDERED_ACCESS, CRUDE_GFX_RHI_RESOURCE_STATE_INDIRECT_ARGUMENT );
    crude_gfx_cmd_add_buffer_barrier( primary_cmd, pass->culled_pointshadow_meshletes_instances_hga.buffer_handle, CRUDE_GFX_RHI_RESOURCE_STATE_UNORDERED_ACCESS, CRUDE_GFX_RHI_RESOURCE_STATE_SHADER_RESOURCE );
  ...
  /* Pointshadow Commands Generation */
  {
    ...
    crude_gfx_cmd_add_buffer_barrier( primary_cmd, pass->culled_pointshadow_meshlet_draw_commands_hga.buffer_handle, CRUDE_GFX_RHI_RESOURCE_STATE_INDIRECT_ARGUMENT, CRUDE_GFX_RHI_RESOURCE_STATE_UNORDERED_ACCESS );
    ...
    push_constant.scene = pass->scene_renderer->scene_hga.gpu_address;
    push_constant.culled_pointshadow_meshletes_instances_count = pass->culled_pointshadow_meshletes_instances_count_hga.gpu_address;
    push_constant.culled_pointshadow_meshlet_draw_commands = pass->culled_pointshadow_meshlet_draw_commands_hga.gpu_address;
    crude_gfx_cmd_push_constant( primary_cmd, &push_constant, sizeof( push_constant ) );
    ...
    crude_gfx_cmd_add_buffer_barrier( primary_cmd, pass->culled_pointshadow_meshlet_draw_commands_hga.buffer_handle, CRUDE_GFX_RHI_RESOURCE_STATE_UNORDERED_ACCESS, CRUDE_GFX_RHI_RESOURCE_STATE_INDIRECT_ARGUMENT );
    crude_gfx_cmd_pop_marker( primary_cmd );
  }
  
  tetrahedron_shadow_texture = crude_gfx_access_texture( gpu, pass->culled_tetrahedron_shadow_texture );
  ...
  /* GPU Pro 6 Tile-Based Omnidirectional Shadows Hawar Doghramachi, mixed with http://www.hd-prg.com/tileBasedShadows.html */
  {
    ...
    pointlight_spheres_tca = crude_gfx_linear_allocator_allocate( &gpu->frame_linear_allocator, pass->culled_pointlight_spheres_hga.size );
    pointlight_spheres = CRUDE_CAST( XMFLOAT4*, pointlight_spheres_tca.cpu_address );

    pointlight_world_to_clip_tca = crude_gfx_linear_allocator_allocate( &gpu->frame_linear_allocator, pass->scene_renderer->culled_lights_world_to_texture_hga.size );
    ...
    if ( pointlight_spheres && pointlight_world_to_clip )
    {
      ...
      crude_gfx_cmd_memory_copy( primary_cmd, pointlight_world_to_clip_tca, pass->scene_renderer->culled_lights_world_to_texture_hga, 0, 0 );
      crude_gfx_cmd_memory_copy( primary_cmd, pointlight_spheres_tca, pass->culled_pointlight_spheres_hga, 0, 0 );
    }
    
    crude_gfx_cmd_push_marker( primary_cmd, "pointshadow_draw_pass" );

    crude_gfx_cmd_add_image_barrier( primary_cmd, pass->culled_tetrahedron_shadow_texture, CRUDE_GFX_RHI_RESOURCE_STATE_DEPTH_WRITE, 0u, 1u, true );
    ...
    push_constant.culled_pointshadow_meshletes_instances = pass->culled_pointshadow_meshletes_instances_hga.gpu_address;
    push_constant.culled_pointshadow_meshletes_instances_count = pass->culled_pointshadow_meshletes_instances_count_hga.gpu_address;
    push_constant.culled_pointlight_spheres = pass->culled_pointlight_spheres_hga.gpu_address;
    push_constant.culled_pointshadow_meshlet_draw_commands = pass->culled_pointshadow_meshlet_draw_commands_hga.gpu_address;
    ...
    crude_gfx_cmd_draw_mesh_task_indirect_count( primary_cmd,
      pass->culled_pointshadow_meshlet_draw_commands_hga.buffer_handle, 0u,
      pass->culled_pointshadow_meshletes_instances_count_hga.buffer_handle, sizeof( uint32 ) * CRUDE_LIGHTS_MAX_COUNT,
      4u * CRUDE_LIGHTS_MAX_COUNT, sizeof( XMUINT4 ) );
  
    crude_gfx_cmd_end_render_pass( primary_cmd );
  
    crude_gfx_cmd_add_image_barrier( primary_cmd, pass->culled_tetrahedron_shadow_texture, CRUDE_GFX_RHI_RESOURCE_STATE_PIXEL_SHADER_RESOURCE, 0u, 1u, true );
    crude_gfx_cmd_pop_marker( primary_cmd );
  }
}
