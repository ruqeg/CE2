...
#define DEPTH_PYRAMID_FARTHEST
#include <engine/graphics/shaders/depth_pyramid.crude_shader>
...
(
  ...
  _In_ crude_gfx_depth_pyramid_type                        type
)
{
  ...
  sampler_creation.reduction_mode = ( type == CRUDE_GFX_DEPTH_PYRAMID_TYPE_FARTHEST ) ? CRUDE_GFX_RHI_SAMPLER_REDUCTION_MODE_MAX : CRUDE_GFX_RHI_SAMPLER_REDUCTION_MODE_MIN;
  sampler_creation.name = CRUDE_STRING_VIEW_MAKE( "depth_pyramid_sampler" );
...
)
{
  ...
  pass = CRUDE_CAST( crude_gfx_depth_pyramid_pass*, ctx );
  gpu = pass->scene_renderer->gpu;

  if ( pass->type == CRUDE_GFX_DEPTH_PYRAMID_TYPE_FARTHEST )
  {
    depth_pyramid_pipeline = CRUDE_GFX_ACCESS_TECHNIQUE_PIPELINE_BY_NAME( "depth_pyramid_farthest" );
  }
  else
  {
    depth_pyramid_pipeline = CRUDE_GFX_ACCESS_TECHNIQUE_PIPELINE_BY_NAME( "depth_pyramid_nearest" );
  }
...
)
{
  ...
  crude_string_raw_copy( depth_hierarchy_creation.name, ( pass->type == CRUDE_GFX_DEPTH_PYRAMID_TYPE_FARTHEST ) ? "depth_hierarchy_farthest" : "depth_hierarchy_nearest", sizeof( depth_hierarchy_creation.name ) );
  ...
  depth_pyramid_view_creation.name = ( pass->type == CRUDE_GFX_DEPTH_PYRAMID_TYPE_FARTHEST ) ? CRUDE_STRING_VIEW_MAKE( "depth_pyramid_view_farthest" ) : CRUDE_STRING_VIEW_MAKE( "depth_pyramid_view_nearest" );
