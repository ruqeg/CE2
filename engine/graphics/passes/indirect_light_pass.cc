...
)
{
  ...
  CRUDE_STRING_RAW_COPY( texture_creation.name, "probe_rt_radiance" );
  ...
  texture_creation.type = CRUDE_GFX_TEXTURE_TYPE_TEXTURE_2D;
  texture_creation.flags = CRUDE_GFX_TEXTURE_MASK_COMPUTE;
  CRUDE_STRING_RAW_COPY( texture_creation.name, "probe_irradiance" );
  ...
  texture_creation.type = CRUDE_GFX_TEXTURE_TYPE_TEXTURE_2D;
  texture_creation.flags = CRUDE_GFX_TEXTURE_MASK_COMPUTE;
  CRUDE_STRING_RAW_COPY( texture_creation.name, "probe_visibility" );
  ...
  sampler_creation.name = CRUDE_STRING_VIEW_MAKE( "probe_grid_sampler_handle" );
  ...
  texture_creation.type = CRUDE_GFX_TEXTURE_TYPE_TEXTURE_2D;
  texture_creation.flags = CRUDE_GFX_TEXTURE_MASK_COMPUTE;
  CRUDE_STRING_RAW_COPY( texture_creation.name, "probe_offsets" );
...
)
{
  ...
  pass = CRUDE_CAST( crude_gfx_indirect_light_pass*, ctx );
  ...
  /* Probe Raytracer */
  {
    ...
    pipeline = CRUDE_GFX_ACCESS_TECHNIQUE_PIPELINE_BY_NAME( "probe_raytracer" );
  ...
  if ( pass->offsets_calculations_count >= 0 )
  {
    ...
    pipeline = CRUDE_GFX_ACCESS_TECHNIQUE_PIPELINE_BY_NAME( "calculate_probe_offsets" );
  ...
  /* Probe Update Irradiance */
  {
    ...
    pipeline = CRUDE_GFX_ACCESS_TECHNIQUE_PIPELINE_BY_NAME( "probe_update_irradiance" );
  ...
  /* Probe Update Visibility */
  {
    ...
    pipeline = CRUDE_GFX_ACCESS_TECHNIQUE_PIPELINE_BY_NAME( "probe_update_visibility" );
...
#if CRUDE_GFX_INDIRECT_LIGH_PASS_HALF_RES
    width = gpu->renderer_size.x * 0.5f;
    height = gpu->renderer_size.y * 0.5f;
#else
    width = gpu->renderer_size.x;
    height = gpu->renderer_size.y;
#endif

    pipeline = CRUDE_GFX_ACCESS_TECHNIQUE_PIPELINE_BY_NAME( "sample_irradiance" );
