...
)
{
  ...
  /* SSR Blurring Pass  */
  {
    ...
    /* Convolve Vertical */
    {
      ...
      pipeline = crude_gfx_access_technique_pipeline_by_name( gpu, "ssr_convolve_vertical", 0u );
      crude_gfx_cmd_bind_pipeline( primary_cmd, pipeline );
...
)
{
  ...
  crude_string_copy( radiance_hierarchy_creation.name, "radiance_hierarchy_texture", sizeof( radiance_hierarchy_creation.name ) );
  ...
  crude_string_copy( radiance_hierarchy_creation.name, "radiance_hierarchy_temporary_texture", sizeof( radiance_hierarchy_creation.name ) );
  ...
  for ( uint32 i = 0; i < pass->radiance_hierarchy_levels; ++i )
  {
    ...
    radiance_hierarchy_view_creation.name = "radiance_hierarchy_views";
  ...
  for ( uint32 i = 0; i < pass->radiance_hierarchy_levels - 1; ++i )
  {
    ...
    radiance_hierarchy_view_creation.view_type = CRUDE_GFX_RHI_IMAGE_VIEW_TYPE_2D;
    radiance_hierarchy_view_creation.subresource.mip_base_level = i;
    radiance_hierarchy_view_creation.name = "radiance_hierarchy_temporary_views";
