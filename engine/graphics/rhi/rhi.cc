...
(
  ...
  _In_ crude_string_view const                             object_name
)
{
...
{
  return g_rhi_.crude_gfx_rhi_get_device_name( device );
}
...
)
{
  ...
  char                                                     vk_env[ 1024 ];
  crude_string_view                                        glsl_compiler_absolute_filepath;
  crude_string_view                                        glsl_compiler_arguments;
  crude_string_view                                        glsl_compiler_optional_arguments;
  crude_string_view                                        spirv_optimizer_absolute_filepath;
  crude_string_view                                        spirv_optimizer_arguments;
  crude_string_view                                        spirv_optimized_absolute_filepath;
  crude_string_view                                        spirv_debug_absolute_filepath;
  crude_string_view                                        spirv_absolute_filepath_ref;
  crude_string_buffer                                      temporary_string_buffer;
  crude_string_view                                        vk_binaries_path;
  crude_string_view                                        glsl_absolute_filepath;
  crude_string_view                                        pass_name;
  crude_string_view                                        compiler_extension;
  uint32                                                   i;

  pass_name = crude_string_view_valid( desc->pass_name ) ? desc->pass_name : CRUDE_STRING_VIEW_MAKE( "unknown" );
  compiler_extension = crude_gfx_rhi_shader_stage_to_compiler_extension( desc->stage );

  crude_string_buffer_initialize( &temporary_string_buffer, CRUDE_RKILO( 2 ), crude_heap_allocator_pack( allocator ) );

  glsl_absolute_filepath = crude_string_buffer_append_use_f( &temporary_string_buffer, "%s\\%s.%s",
    desc->temporary_absolute_directory.data, pass_name.data, compiler_extension.data );

  crude_write_file( glsl_absolute_filepath, desc->code );

  define_name_upper[ 0 ] = 0;
  for ( i = 0; i < desc->define_name.length; i++ )
  {
    define_name_upper[ i ] = toupper( desc->define_name.data[ i ] );
  }
  ...
  glsl_compiler_absolute_filepath = crude_string_buffer_append_use_f(
    ...
    vk_binaries_path.data );

  spirv_debug_absolute_filepath = crude_string_buffer_append_use_f(
    &temporary_string_buffer,
    "%s\\%s",
    desc->temporary_absolute_directory.data,
    "spirv_debug.spv" ); 
  
  if ( desc->optimized )
  {
    glsl_compiler_optional_arguments = CRUDE_STRING_VIEW_MAKE( "" );
   }
  else
  {
#if CRUDE_DEVELOP
    glsl_compiler_optional_arguments = CRUDE_STRING_VIEW_MAKE( "-gVS --D CRUDE_DEVELOP=1" );
#else /* CRUDE_DEVELOP */
    glsl_compiler_optional_arguments = CRUDE_STRING_VIEW_MAKE( "-gVS" );
#endif /* CRUDE_DEVELOP */
  }
  ...
  glsl_compiler_arguments = crude_string_buffer_append_use_f(
    ...
    glsl_absolute_filepath.data,
    spirv_debug_absolute_filepath.data,
    compiler_extension.data,
    crude_gfx_rhi_shader_stage_to_defines( desc->stage ).data,
    define_name_upper,
    crude_gfx_rhi_current_graphics_api_str( ).data,
    "CRUDE_FLAGS", 
    desc->define_flags,
    glsl_compiler_optional_arguments.data );

#else /*_MSC_VER */
  CRUDE_ASSERT( false );
#endif /*_MSC_VER */

  CRUDE_ASSERT( crude_process_execute( ".", glsl_compiler_absolute_filepath.data, glsl_compiler_arguments.data, "ERROR" ) );
  ...
  if ( desc->optimized )
  {
    spirv_optimized_absolute_filepath = crude_string_buffer_append_use_f(
      ...
      desc->compiled_absolute_directory.data,
      pass_name.data,
      compiler_extension.data );

    spirv_optimizer_absolute_filepath = crude_string_buffer_append_use_f( &temporary_string_buffer, "%sspirv-opt.exe", vk_binaries_path.data );
    spirv_optimizer_arguments = crude_string_buffer_append_use_f( &temporary_string_buffer, "spirv-opt.exe --preserve-bindings --relax-block-layout --scalar-block-layout --preserve-spec-constants -O %s -o %s", spirv_debug_absolute_filepath.data, spirv_optimized_absolute_filepath.data );

    CRUDE_ASSERT( crude_process_execute( ".", spirv_optimizer_absolute_filepath.data, spirv_optimizer_arguments.data, "ERROR" ) );
  }
  ...
  *spirv_absolute_filepath = CRUDE_CAST( char*, crude_heap_allocator_allocate( allocator, spirv_absolute_filepath_ref.length + 1 ) );
  crude_string_raw_copy( *spirv_absolute_filepath, spirv_absolute_filepath_ref.data, spirv_absolute_filepath_ref.length );
  (*spirv_absolute_filepath)[ spirv_absolute_filepath_ref.length ] = 0;
...
)
{
  switch ( value )
  {
  case CRUDE_GFX_RHI_SHADER_STAGE_VERTEX_BIT:          return CRUDE_STRING_VIEW_MAKE( "vert" );
  case CRUDE_GFX_RHI_SHADER_STAGE_MESH_BIT_EXT:        return CRUDE_STRING_VIEW_MAKE( "mesh" );
  case CRUDE_GFX_RHI_SHADER_STAGE_TASK_BIT_EXT:        return CRUDE_STRING_VIEW_MAKE( "task" );
  case CRUDE_GFX_RHI_SHADER_STAGE_FRAGMENT_BIT:        return CRUDE_STRING_VIEW_MAKE( "frag" );
  case CRUDE_GFX_RHI_SHADER_STAGE_COMPUTE_BIT:         return CRUDE_STRING_VIEW_MAKE( "comp" );
  case CRUDE_GFX_RHI_SHADER_STAGE_CLOSEST_HIT_BIT_KHR: return CRUDE_STRING_VIEW_MAKE( "rchit" );
  case CRUDE_GFX_RHI_SHADER_STAGE_RAYGEN_BIT_KHR:      return CRUDE_STRING_VIEW_MAKE( "rgen" );
  case CRUDE_GFX_RHI_SHADER_STAGE_MISS_BIT_KHR:        return CRUDE_STRING_VIEW_MAKE( "rmiss" );
  }
  return CRUDE_STRING_VIEW_MAKE( "" );
}

crude_gfx_rhi_blend_op
crude_gfx_rhi_string_to_blend_op
(
  _In_ crude_string_view const                             op
)
{
  if ( CRUDE_STRING_VIEW_CMP( op, "ADD" ) == 0 )               return CRUDE_GFX_RHI_BLEND_OP_ADD;
  if ( CRUDE_STRING_VIEW_CMP( op, "SUBTRACT" ) == 0 )          return CRUDE_GFX_RHI_BLEND_OP_SUBTRACT;
  if ( CRUDE_STRING_VIEW_CMP( op, "REVERSE_SUBTRACT" ) == 0 )  return CRUDE_GFX_RHI_BLEND_OP_REVERSE_SUBTRACT;
  if ( CRUDE_STRING_VIEW_CMP( op, "MIN" ) == 0 )               return CRUDE_GFX_RHI_BLEND_OP_MIN;
  if ( CRUDE_STRING_VIEW_CMP( op, "MAX" ) == 0 )               return CRUDE_GFX_RHI_BLEND_OP_MAX;
...
)
{
  switch ( value )
  {
  case CRUDE_GFX_RHI_SHADER_STAGE_VERTEX_BIT:          return CRUDE_STRING_VIEW_MAKE( "CRUDE_STAGE_VERTEX" );
  case CRUDE_GFX_RHI_SHADER_STAGE_FRAGMENT_BIT:        return CRUDE_STRING_VIEW_MAKE( "CRUDE_STAGE_FRAGMENT" );
  case CRUDE_GFX_RHI_SHADER_STAGE_COMPUTE_BIT:         return CRUDE_STRING_VIEW_MAKE( "CRUDE_STAGE_COMPUTE" );
  case CRUDE_GFX_RHI_SHADER_STAGE_MESH_BIT_EXT:        return CRUDE_STRING_VIEW_MAKE( "CRUDE_STAGE_MESH" );
  case CRUDE_GFX_RHI_SHADER_STAGE_TASK_BIT_EXT:        return CRUDE_STRING_VIEW_MAKE( "CRUDE_STAGE_TASK" );
  case CRUDE_GFX_RHI_SHADER_STAGE_CLOSEST_HIT_BIT_KHR: return CRUDE_STRING_VIEW_MAKE( "CRUDE_CLOSEST_HIT" );
  case CRUDE_GFX_RHI_SHADER_STAGE_RAYGEN_BIT_KHR:      return CRUDE_STRING_VIEW_MAKE( "CRUDE_RAYGEN" );
  case CRUDE_GFX_RHI_SHADER_STAGE_MISS_BIT_KHR:        return CRUDE_STRING_VIEW_MAKE( "CRUDE_MISS" );
  }
  return CRUDE_STRING_VIEW_MAKE( "" );
}
...
)
{
  switch ( value )
  {
  case CRUDE_GFX_RHI_SHADER_STAGE_VERTEX_BIT:          return CRUDE_STRING_VIEW_MAKE( "vs_6_0" );
  case CRUDE_GFX_RHI_SHADER_STAGE_MESH_BIT_EXT:        return CRUDE_STRING_VIEW_MAKE( "ms_6_0" );
  case CRUDE_GFX_RHI_SHADER_STAGE_TASK_BIT_EXT:        return CRUDE_STRING_VIEW_MAKE( "as_6_0" );
  case CRUDE_GFX_RHI_SHADER_STAGE_FRAGMENT_BIT:        return CRUDE_STRING_VIEW_MAKE( "ps_6_0" );
  case CRUDE_GFX_RHI_SHADER_STAGE_COMPUTE_BIT:         return CRUDE_STRING_VIEW_MAKE( "cs_6_0" );
  case CRUDE_GFX_RHI_SHADER_STAGE_CLOSEST_HIT_BIT_KHR: return CRUDE_STRING_VIEW_MAKE( "chit_6_0" );
  case CRUDE_GFX_RHI_SHADER_STAGE_RAYGEN_BIT_KHR:      return CRUDE_STRING_VIEW_MAKE( "rgen_6_0" );
  case CRUDE_GFX_RHI_SHADER_STAGE_MISS_BIT_KHR:        return CRUDE_STRING_VIEW_MAKE( "rmiss_6_0" );
  }
  return CRUDE_STRING_VIEW_MAKE( "" );
}