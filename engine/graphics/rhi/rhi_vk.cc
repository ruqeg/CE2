...
{
#if CRUDE_GRAPHICS_VALIDATION_LAYERS_ENABLED
  CRUDE_ASSERT( crude_string_view_valid( name ) );
  vmaSetAllocationName( device->vk.vma_allocator, image.vk.vma_allocation, name.data );
#endif /* CRUDE_GRAPHICS_VALIDATION_LAYERS_ENABLED */
}
...
)
{
  ...
  {
    CRUDE_ASSERT( crude_string_view_valid( object_name ) );
    ...
    vk_name_info.pObjectName  = object_name.data;
...
)
{
  ...
  return crude_string_view_make( vk_physical_properties.deviceName ? vk_physical_properties.deviceName : "Unknown" );
}
...
{
  return CRUDE_STRING_VIEW_MAKE( "Vulkan" );
}
...
)
{
  ...
  *vk_selected_physical_devices = VK_NULL_HANDLE;
  ...
  for ( size_t i = 0; i < vk_available_extensions_count; ++i )
  {
    ...
    if ( crude_string_raw_cmp( vk_available_extensions[ i ].extensionName, VK_EXT_MESH_SHADER_EXTENSION_NAME ) == 0 )
    {
    ...
    if ( crude_string_raw_cmp( vk_available_extensions[ i ].extensionName, VK_KHR_FRAGMENT_SHADING_RATE_EXTENSION_NAME ) == 0 )
    {
    ...
    if ( crude_string_raw_cmp( vk_available_extensions[ i ].extensionName, VK_KHR_DEFERRED_HOST_OPERATIONS_EXTENSION_NAME ) == 0 )
    {
    ...
    if ( crude_string_raw_cmp( vk_available_extensions[ i ].extensionName, VK_KHR_SHADER_RELAXED_EXTENDED_INSTRUCTION_EXTENSION_NAME ) == 0 )
    {
