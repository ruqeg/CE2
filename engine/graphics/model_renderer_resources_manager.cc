...
)
{
  ...
  if ( crude_gfx_rhi_get_type( ) == CRUDE_GFX_RHI_TYPE_VULKAN )
  {
    ...
    manager->bottom_level_acceleration_structure_transform_hga = crude_gfx_memory_allocate_with_pname( manager->gpu, sizeof( vk_transform_matrix ), CRUDE_GFX_MEMORY_TYPE_CPU_GPU, "bottom_level_acceleration_structure_transform_hga", CRUDE_GFX_RHI_BUFFER_USAGE_ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY_BIT_KHR );
  ...
  {
    manager->bottom_level_acceleration_structure_transform_hga = crude_gfx_memory_allocate_with_pname( manager->gpu, sizeof( XMFLOAT4X4 ), CRUDE_GFX_MEMORY_TYPE_CPU_GPU, "bottom_level_acceleration_structure_transform_hga", CRUDE_GFX_RHI_BUFFER_USAGE_ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY_BIT_KHR );
  }
  else
  {
    CRUDE_ASSERT( false );
...
)
{
  for ( uint32 image_index = 0; image_index < gltf->images_count; ++image_index )
  {
    ...
    char                                                  *image_relative_filepath;
    int                                                    comp, width, height;
    
    image = &gltf->images[ image_index ];
    image_relative_filepath = crude_string_buffer_append_use_f(
      &manager->image_absolute_filepath_string_buffer,
      "%s%s",
      gltf_directory + manager->gpu->environment->directories.resources_absolute_directory_length - 1,
      image->uri );
    texture_handle = crude_gfx_texture_manager_get_texture( manager->texture_manager, image_relative_filepath );
...
)
{
  ...
  if ( manager->gpu->mesh_shaders_extension_present )
  {
    ...
    /* Create indices buffer for meshes (so we still can use meshlets vertices ) */
    {
      ...
      for ( uint32 gltf_mesh_index = 0; gltf_mesh_index < gltf->meshes_count; ++gltf_mesh_index )
      {
        ...
        for ( uint32 gltf_primitive_index = 0; gltf_primitive_index < gltf_mesh->primitives_count; ++gltf_primitive_index )
        {
          ...
          mesh_cpu->index_hga = crude_gfx_memory_allocate_with_pname(
            manager->gpu,
    ...
    /* Wee need it for blas creation, so create and copy buffer right now (no async ) */
    {
      ...
      gpu_allocation = crude_gfx_memory_allocate_with_pname(
        manager->gpu,
    ...
    *node = CRUDE_COMPOUNT_EMPTY( crude_gfx_node );
    ...
    if ( gltf_node->mesh && gltf_node->skin )
    {
      ...
      for ( uint32 pi = 0; pi < gltf_node->mesh->primitives_count; ++pi )
      {
        ...
        CRUDE_ARRAY_INITIALIZE_WITH_LENGTH( node->affected_joints[ pi ], 0, crude_heap_allocator_pack( manager->allocator ) );
        CRUDE_ARRAY_INITIALIZE_WITH_LENGTH( node->affected_joints_local_aabb[ pi ], 0, crude_heap_allocator_pack( manager->allocator ) );
...
)
{
  ...
  for ( uint32 i = 0; i < CRUDE_ARRAY_LENGTH( model_renderer_resources->meshes ); ++i )
  {
    ...
    acceleration_structure_geometries[ i ].geometry.triangles.max_vertex = manager->total_meshlets_vertices_count;
    ...
    model_renderer_resources->blases_hga[ i ] = crude_gfx_memory_allocate_with_pname( manager->gpu, acceleration_structure_build_sizes_info.acceleration_structure_size, CRUDE_GFX_MEMORY_TYPE_GPU, "blas_hga", CRUDE_GFX_RHI_BUFFER_USAGE_ACCELERATION_STRUCTURE_STORAGE_BIT_KHR );
