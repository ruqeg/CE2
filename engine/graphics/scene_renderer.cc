...
)
{
  ...
  scene_renderer->options.depth_pyramid_pass.depth = CRUDE_STRING_VIEW_MAKE( "depth" );
  
  scene_renderer->options.ssr_pass.depth_texture = CRUDE_STRING_VIEW_MAKE( "depth" );
  scene_renderer->options.ssr_pass.normal_texture = CRUDE_STRING_VIEW_MAKE( "direct_normal" );
  scene_renderer->options.ssr_pass.roughness_metalness_texture = CRUDE_STRING_VIEW_MAKE( "direct_roughness_metalness" );
  scene_renderer->options.ssr_pass.ssr_hit_uv_depth_rdotv_texture = CRUDE_STRING_VIEW_MAKE( "ssr_hit_uv_depth_rdotv" );
  scene_renderer->options.ssr_pass.ssr_texture = CRUDE_STRING_VIEW_MAKE( "ssr" );
  scene_renderer->options.ssr_pass.direct_radiance_texture = CRUDE_STRING_VIEW_MAKE( "direct_radiance" );
  
  scene_renderer->options.compose_pass.direct_radiance_texture = CRUDE_STRING_VIEW_MAKE( "direct_radiance" );
  scene_renderer->options.compose_pass.indirect_radiance_texture = CRUDE_STRING_VIEW_MAKE( "indirect_radiance" );
  scene_renderer->options.compose_pass.radiance_texture = CRUDE_STRING_VIEW_MAKE( "radiance" );
  scene_renderer->options.compose_pass.albedo_texture = CRUDE_STRING_VIEW_MAKE( "direct_albedo" );
  scene_renderer->options.compose_pass.roughness_metalness_texture = CRUDE_STRING_VIEW_MAKE( "direct_roughness_metalness" );
  scene_renderer->options.compose_pass.ssr_texture = CRUDE_STRING_VIEW_MAKE( "ssr" );

  scene_renderer->options.postprocessing_pass.hdr_pre_tonemapping = CRUDE_STRING_VIEW_MAKE( "radiance" );
  scene_renderer->options.postprocessing_pass.gamma = 2.2;
   
  scene_renderer->world_environment = crude_world_environment_empty( );

  scene_renderer->options.indirect_light.depth_texture = CRUDE_STRING_VIEW_MAKE( "depth" );
  scene_renderer->options.indirect_light.normal_texture = CRUDE_STRING_VIEW_MAKE( "direct_normal" );
  scene_renderer->options.indirect_light.indirect_radiance_texture = CRUDE_STRING_VIEW_MAKE( "indirect_radiance" );
  ...
  crude_gfx_depth_pyramid_pass_initialize( &scene_renderer->depth_pyramid_nearest_pass, scene_renderer, CRUDE_GFX_DEPTH_PYRAMID_TYPE_NEAREST );
  crude_gfx_depth_pyramid_pass_initialize( &scene_renderer->depth_pyramid_farthest_pass, scene_renderer, CRUDE_GFX_DEPTH_PYRAMID_TYPE_FARTHEST );
...
{
  crude_gfx_rhi_wait_idle( &scene_renderer->gpu->rhi_device );

  crude_gfx_render_graph_builder_unregister_all_render_passes( scene_renderer->render_graph->builder );
#if CRUDE_GFX_RAY_TRACING_SOLID_DEBUG_ENABLED
  crude_gfx_render_graph_builder_unregister_render_pass( scene_renderer->render_graph->builder, CRUDE_STRING_VIEW_MAKE( "ray_tracing_solid_pass" ) );
#endif
#if CRUDE_GFX_RAY_TRACING_DDGI_ENABLED
  crude_gfx_render_graph_builder_unregister_render_pass( scene_renderer->render_graph->builder, CRUDE_STRING_VIEW_MAKE( "indirect_light_pass" ) );
  crude_gfx_render_graph_builder_unregister_render_pass( scene_renderer->render_graph->builder, CRUDE_STRING_VIEW_MAKE( "indirect_light_debug_pass" ) );
#endif
  ...
  crude_gfx_depth_pyramid_pass_deinitialize( &scene_renderer->depth_pyramid_nearest_pass );
  crude_gfx_depth_pyramid_pass_deinitialize( &scene_renderer->depth_pyramid_farthest_pass );
...
)
{
  ...
  for ( uint32 i = 0; i < CRUDE_ARRAY_LENGTH( scene_renderer->model_renderer_resoruces_instances ); ++i )
  {
    ...
    model_renderer_resources = crude_gfx_model_renderer_resources_manager_access( scene_renderer->model_renderer_resources_manager, scene_renderer->model_renderer_resoruces_instances[ i ].model_renderer_resources_handle );
...
)
{
  ...
  CRUDE_GFX_CMD_PUSH_MARKER( scene_renderer->primary_cmd, "update_dynamic_buffers" );
...
)
{
  ...
  {
#if CRUDE_EDITOR
    crude_gfx_render_graph_builder_register_render_pass( render_graph->builder, CRUDE_STRING_VIEW_MAKE( "imgui_editor_pass" ), crude_gfx_imgui_pass_pack( &scene_renderer->imgui_pass ) );
#else
    crude_gfx_render_graph_builder_register_render_pass( render_graph->builder, crude_string_view_make( "imgui_game_pass" ), crude_gfx_imgui_pass_pack( &scene_renderer->imgui_pass ) );
#endif
  }

  crude_gfx_render_graph_builder_register_render_pass( render_graph->builder, CRUDE_STRING_VIEW_MAKE( "opaque_early_pass" ), crude_gfx_opaque_early_pass_pack( &scene_renderer->opaque_early_pass ) );
  crude_gfx_render_graph_builder_register_render_pass( render_graph->builder, CRUDE_STRING_VIEW_MAKE( "opaque_late_pass" ), crude_gfx_opaque_late_pass_pack( &scene_renderer->opaque_late_pass ) );
  crude_gfx_render_graph_builder_register_render_pass( render_graph->builder, CRUDE_STRING_VIEW_MAKE( "depth_pyramid_farthest_pass" ), crude_gfx_depth_pyramid_pass_pack( &scene_renderer->depth_pyramid_farthest_pass ) );
  crude_gfx_render_graph_builder_register_render_pass( render_graph->builder, CRUDE_STRING_VIEW_MAKE( "depth_pyramid_nearest_pass" ), crude_gfx_depth_pyramid_pass_pack( &scene_renderer->depth_pyramid_nearest_pass ) );
  crude_gfx_render_graph_builder_register_render_pass( render_graph->builder, CRUDE_STRING_VIEW_MAKE( "culling_early_pass" ), crude_gfx_culling_early_pass_pack( &scene_renderer->culling_early_pass ) );
  crude_gfx_render_graph_builder_register_render_pass( render_graph->builder, CRUDE_STRING_VIEW_MAKE( "culling_late_pass" ), crude_gfx_culling_late_pass_pack( &scene_renderer->culling_late_pass ) );
  crude_gfx_render_graph_builder_register_render_pass( render_graph->builder, CRUDE_STRING_VIEW_MAKE( "debug_pass" ), crude_gfx_debug_pass_pack( &scene_renderer->debug_pass ) );
  crude_gfx_render_graph_builder_register_render_pass( render_graph->builder, CRUDE_STRING_VIEW_MAKE( "compose_pass" ), crude_gfx_compose_pass_pack( &scene_renderer->compose_pass ) );
  crude_gfx_render_graph_builder_register_render_pass( render_graph->builder, CRUDE_STRING_VIEW_MAKE( "postprocessing_pass" ), crude_gfx_postprocessing_pass_pack( &scene_renderer->postprocessing_pass ) );
  crude_gfx_render_graph_builder_register_render_pass( render_graph->builder, CRUDE_STRING_VIEW_MAKE( "translucent_pass" ), crude_gfx_translucent_pass_pack( &scene_renderer->translucent_pass ) );
  crude_gfx_render_graph_builder_register_render_pass( render_graph->builder, CRUDE_STRING_VIEW_MAKE( "opaque_terrain_pass" ), crude_gfx_opaque_terrain_pass_pack( &scene_renderer->opaque_terrain_pass ) );
  crude_gfx_render_graph_builder_register_render_pass( render_graph->builder, CRUDE_STRING_VIEW_MAKE( "light_lut_pass" ), crude_gfx_light_lut_pass_pack( &scene_renderer->light_lut_pass ) );
  crude_gfx_render_graph_builder_register_render_pass( render_graph->builder, CRUDE_STRING_VIEW_MAKE( "ssr_pass" ), crude_gfx_ssr_pass_pack( &scene_renderer->ssr_pass ) );
  crude_gfx_render_graph_builder_register_render_pass( render_graph->builder, CRUDE_STRING_VIEW_MAKE( "volumetric_fog_pass" ), crude_gfx_volumetric_fog_pass_pack( &scene_renderer->volumetric_fog_pass ) );
  crude_gfx_render_graph_builder_register_render_pass( render_graph->builder, CRUDE_STRING_VIEW_MAKE( "pointlight_shadows_pass" ), crude_gfx_pointlight_shadow_pass_pack( &scene_renderer->pointlight_shadow_pass ) );
#if CRUDE_GFX_RAY_TRACING_SOLID_DEBUG_ENABLED
  crude_gfx_render_graph_builder_register_render_pass( render_graph->builder, CRUDE_STRING_VIEW_MAKE( "ray_tracing_solid_pass" ), crude_gfx_ray_tracing_solid_pass_pack( &scene_renderer->ray_tracing_solid_pass ) );
#endif

#if CRUDE_GFX_RAY_TRACING_DDGI_ENABLED
  crude_gfx_render_graph_builder_register_render_pass( render_graph->builder, crude_string_view_make( "indirect_light_pass" ), crude_gfx_indirect_light_pass_pack( &scene_renderer->indirect_light_pass ) );
  crude_gfx_render_graph_builder_register_render_pass( render_graph->builder, crude_string_view_make( "indirect_light_debug_pass" ), crude_gfx_indirect_light_debug_pass_pack( &scene_renderer->indirect_light_debug_pass ) );
  crude_gfx_indirect_light_pass_on_disabled( &scene_renderer->indirect_light_pass );
#endif
}
...
)
{
...
#endif
    scene->background_radiance.x = scene_renderer->world_environment.background_color.x * scene_renderer->world_environment.background_intencity;
    scene->background_radiance.y = scene_renderer->world_environment.background_color.y * scene_renderer->world_environment.background_intencity;
    scene->background_radiance.z = scene_renderer->world_environment.background_color.z * scene_renderer->world_environment.background_intencity;
    scene->ambient_radiance.x = scene_renderer->world_environment.ambient_color.x * scene_renderer->world_environment.ambient_intencity;
    scene->ambient_radiance.y = scene_renderer->world_environment.ambient_color.y * scene_renderer->world_environment.ambient_intencity;
    scene->ambient_radiance.z = scene_renderer->world_environment.ambient_color.z * scene_renderer->world_environment.ambient_intencity;
  ...
  /* Update meshes instanse draws & joints matrices buffers */
  {
    ...
    for ( uint32 model_instance_index = 0u; model_instance_index < CRUDE_ARRAY_LENGTH( scene_renderer->model_renderer_resoruces_instances ); ++model_instance_index )
    {
      ...
      model_renderer_resources = crude_gfx_model_renderer_resources_manager_access( scene_renderer->model_renderer_resources_manager, model_renderer_resources_instance->model_renderer_resources_handle );
      ...
      for ( uint32 node_index = 0u; node_index < CRUDE_ARRAY_LENGTH( model_renderer_resources->nodes ); ++node_index )
      {
        ...
        if ( node->meshes )
        {
          for ( uint32 mesh_index = 0; mesh_index < CRUDE_ARRAY_LENGTH( node->meshes ); ++mesh_index )
          {
            ...
            crude_gfx_mesh_instance_draw                  *gpu_mesh_instance_draw;

            cpu_mesh = &model_renderer_resources->meshes[ node->meshes[ mesh_index ] ];
            gpu_mesh_instance_draw = &meshes_instances_draws[ scene_renderer->total_visible_meshes_instances_count ]; 

            gpu_mesh_instance_draw->mesh_draw = model_renderer_resources->meshes_draws_hga[ node->meshes[ mesh_index ] ].gpu_address;

            XMStoreFloat4x4( &gpu_mesh_instance_draw->mesh_to_world, mesh_to_world );
            XMStoreFloat4x4( &gpu_mesh_instance_draw->world_to_mesh, XMMatrixInverse( NULL, mesh_to_world ) );
            ...
            if ( node->skin != -1 )
            {
              if ( CRUDE_ARRAY_LENGTH( node->affected_joints[ mesh_index ] ) )
              {
                ...
                XMStoreFloat4( &gpu_mesh_instance_draw->bounding_sphere, XMVectorSet( XMVectorGetX( bounding_center ), XMVectorGetY( bounding_center ), XMVectorGetZ( bounding_center ), bounding_radius ) );
              }
              else
              {
                gpu_mesh_instance_draw->bounding_sphere = cpu_mesh->default_bounding_sphere;
              }

              gpu_mesh_instance_draw->joints_matrices_offset = joints_matrices_offset;
            }
            else
            {
              gpu_mesh_instance_draw->bounding_sphere = cpu_mesh->default_bounding_sphere;
              gpu_mesh_instance_draw->joints_matrices_offset = 0;
            }

            gpu_mesh_instance_draw->flags = 0;

            if ( model_renderer_resources_instance->cast_shadow )
            {
              gpu_mesh_instance_draw->flags |= CRUDE_GFX_MESH_INSTANCE_DRAW_FLAG_CAST_SHADOW;
            }
            
            if ( node->skin != -1 )
            {
              gpu_mesh_instance_draw->flags |= CRUDE_GFX_MESH_INSTANCE_DRAW_FLAG_ANIMATED;
            }
    ...
    if ( CRUDE_ARRAY_LENGTH( scene_renderer->model_renderer_resoruces_instances ) )
    {
  ...
  /* Update meshlets counes storage buffers*/
  {
    ...
    *mesh_draw_counts = CRUDE_COMPOUNT_EMPTY( crude_gfx_mesh_draw_count );
    ...
    mesh_draw_counts->depth_pyramid_farthest_texture_index = scene_renderer->depth_pyramid_farthest_pass.depth_pyramid_texture_handle.index;
...
)
{
  ...
  if ( CRUDE_ENTITY_HAS_COMPONENT( world, node, crude_world_environment ) )
  {
    ...
    scene_renderer->world_environment = *world_environment;
  }
...
)
{
  ...
  {
#if CRUDE_GFX_VULKAN_AVAILABLE
     CRUDE_ARRAY_INITIALIZE_WITH_CAPACITY( vk_acceleration_structure_instances, scene_renderer->total_visible_meshes_instances_count, crude_heap_allocator_pack( scene_renderer->allocator ) );
    ...
    CRUDE_ARRAY_DEINITIALIZE( vk_acceleration_structure_instances );
#else
    CRUDE_ASSERT( false );
#endif /* CRUDE_GFX_VULKAN_AVAILABLE */
  }
  ...
  crude_gfx_rhi_set_acceleration_structure_debug_name( &scene_renderer->gpu->rhi_device, scene_renderer->rhi_tlas, CRUDE_STRING_VIEW_MAKE( "rhi_tlas" ) );
...
)
{
  ...
  ds_creation.name = CRUDE_STRING_VIEW_MAKE( "acceleration_stucture_dsl" );
