...
)
{
  ...
  crude_string_buffer_initialize(
    ...
    environment->resources_absolute_directory.length + relative_filepath.length + 1,
    crude_heap_allocator_pack( manager->allocator ) );

  absolute_filepath = crude_string_buffer_append_use_f( &absolute_filepath_string_buffer, "%s%s", environment->resources_absolute_directory.data, relative_filepath.data );
  crude_string_buffer_close_current_string( &absolute_filepath_string_buffer );

  stbi_info( absolute_filepath.data, &width, &height, &comp );
  ...
  crude_string_raw_copy( texture_creation.name, relative_filepath.data, sizeof( texture_creation.name ) );
  ...
  CRUDE_HASHMAPSTR_SET( manager->texture_relative_filepath_to_handle, crude_string_view_make( texture_creation.name ), texture_handle );
