...
(
  ...
  in uint                                                  depth_pyramid_farthest_texture_index,
...
)
{
  ...
  if ( crude_bounding_sphere_to_clipped_aabb( view_bounding_center, radius, znear, projection_00, projection_11, aabb_clip ) )
  {
    ...
    ivec2 depth_pyramid_size = textureSize( global_textures[ nonuniformEXT( depth_pyramid_farthest_texture_index ) ], 0 );
    ...
    float depth = textureLod( global_textures[ nonuniformEXT( depth_pyramid_farthest_texture_index ) ], uv, level ).r;
    depth = max( depth, textureLod( global_textures[ nonuniformEXT( depth_pyramid_farthest_texture_index ) ], vec2( aabb.x, 1.0f - aabb.y ), level ).r );
    depth = max( depth, textureLod( global_textures[ nonuniformEXT( depth_pyramid_farthest_texture_index ) ], vec2( aabb.z, 1.0f - aabb.w ), level ).r );
    depth = max( depth, textureLod( global_textures[ nonuniformEXT( depth_pyramid_farthest_texture_index ) ], vec2( aabb.x, 1.0f - aabb.w ), level ).r );
    depth = max( depth, textureLod( global_textures[ nonuniformEXT( depth_pyramid_farthest_texture_index ) ], vec2( aabb.z, 1.0f - aabb.y ), level ).r );
...
bool
crude_frustum_culling
(
  in crude_gfx_camera                                      camera,
  in vec3                                                  view_bounding_center,
  in float                                                 view_bounding_radius
)
{
  bool frustum_visible = true;
  for ( uint i = 0; i < 6; ++i )
  {
    frustum_visible = frustum_visible && ( dot( camera.frustum_planes_culling[ i ].xyz, view_bounding_center ) + camera.frustum_planes_culling[ i ].w > -view_bounding_radius );
  }
