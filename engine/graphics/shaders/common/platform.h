...
#define CRUDE_TEXTURE_MIP_COUNT( ti ) textureQueryLevels( global_textures[ nonuniformEXT( ti ) ] )
#define CRUDE_TEXTURE_MIP_SIZE( ti, mip ) textureSize( global_textures[ nonuniformEXT( ti ) ], mip )
...
#define CRUDE_TEXTURE_MIP( ti, uv, mip ) textureLod( global_textures[ nonuniformEXT( ti ) ], uv, mip )
...
vec3 crude_position_from_ndc( vec3 ndc, mat4 clip_to_space )
{
  vec4 h = vec4( ndc, 1.0 );
  vec4 d = h * clip_to_space;
  return d.xyz / d.w;
}

vec3 crude_position_from_depth( vec2 uv, float depth, mat4 clip_to_space )
{
...
{
  return clamp( v, vec2( 0.f ), vec2( 1.f ) );
}
