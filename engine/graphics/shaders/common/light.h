...
)
{
  ...
  float                                                    visiblity, vertex_to_light_distance;
  
  vertex_to_light = light.world_position - vertex_position;
...
)
{
  ...
  direct_radiance.xyz += ( 1.f - metalness ) * albedo.xyz * scene.data.ambient_radiance;
...
#else
      float visibility = crude_calculate_point_light_shadow_contribution( culled_lights.data[ i ], i, vertex_position, scene, culled_lights_world_to_texture );
#endif /* CRUDE_RAYTRACED_SHADOWS */
