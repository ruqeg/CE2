...
(
  _In_ crude_node_manager                                 *manager,
  _In_ crude_string_view const                             node_realtive_filepath,
  _In_ crude_ecs                                          *world
)
{
  ...
  if ( node_index == -1 )
  {
    crude_string_view                                       node_absolute_filepath;
    crude_string_buffer                                     absolute_filepath_string_buffer;

    crude_string_buffer_initialize( &absolute_filepath_string_buffer, manager->environment->resources_absolute_directory.length + node_realtive_filepath.length + 1, crude_heap_allocator_pack( manager->allocator ) );
    node_absolute_filepath = crude_string_buffer_append_use_f( &absolute_filepath_string_buffer, "%s%s", manager->environment->resources_absolute_directory.data, node_realtive_filepath.data );
    ...
    crude_string_raw_copy( new_node_json.relative_filepath, node_realtive_filepath.data, sizeof( new_node_json.relative_filepath ) );

    CRUDE_HASHMAPSTR_SET( manager->relative_filepath_to_node_json, crude_string_view_make( new_node_json.relative_filepath ), new_node_json );
...
(
  ...
  _In_ crude_ecs                                          *world,
  _In_ crude_entity                                        node,
  _In_ crude_string_view const                             saved_relative_filepath
)
{
  cJSON                                                   *node_json;
  char const                                              *node_str;
  crude_string_buffer                                      absolute_filepath_string_buffer;
  crude_string_view                                        saved_absolute_filepath;
  
  crude_string_buffer_initialize( &absolute_filepath_string_buffer, manager->environment->resources_absolute_directory.length + saved_relative_filepath.length + 1, crude_heap_allocator_pack( manager->allocator ) );
  saved_absolute_filepath = crude_string_buffer_append_use_f( &absolute_filepath_string_buffer, "%s%s", manager->environment->resources_absolute_directory.data, saved_relative_filepath.data );

  node_json = crude_node_manager_node_to_json_hierarchy_( manager, world, node );
  node_str = cJSON_Print( node_json );
  crude_write_file( saved_absolute_filepath, crude_string_view_make_with_length( node_str, crude_string_raw_calculate_length( node_str ) ) ); // TODO
...
)
{
  ...
  if ( is_node_external )
  {
    ...
    crude_string_raw_copy( node_external_component.node_relative_filepath, node_external_relative_filepath, sizeof( node_external_component.node_relative_filepath ) );
    node = crude_node_manager_create_node( manager, crude_string_view_make( node_external_relative_filepath ), world );
  ...
  if ( copy_components )
  {
    ...
    for ( uint32 component_index = 0; component_index < cJSON_GetArraySize( node_components_json ); ++component_index )
    {
      ...
      int64 index = CRUDE_HASHMAPSTR_GET_INDEX( manager->components_serialization_manager->component_name_to_json_funs, crude_string_view_make( component_type ) );
...
)
{
  ...
  crude_read_file( absolute_filepath, crude_heap_allocator_pack( manager->allocator ), &json_buffer, &json_buffer_size );
  
  json = cJSON_ParseWithLength( CRUDE_CAST( char const*, json_buffer ), json_buffer_size );
  if ( !json )
  {
...
cleanup:
  if ( json )
  {
    crude_heap_allocator_deallocate( manager->allocator, json_buffer );
  }
  return json;
}
