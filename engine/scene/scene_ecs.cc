...
ECS_COMPONENT_DECLARE( crude_animation_player );
...
CRUDE_COMPONENT_STRING_DEFINE( crude_animation_player, "crude_animation_player" );
...
)
{
  ...
  CRUDE_ECS_COMPONENT_DEFINE( world, crude_animation_player );
  ...
  CRUDE_PARSE_COMPONENT_TO_IMGUI_FUNC_DEFINE( manager, crude_animation_player );
  ...
  CRUDE_PARSE_JSON_TO_COMPONENT_FUNC_DEFINE( manager, crude_animation_player );
  ...
  CRUDE_PARSE_COMPONENT_TO_JSON_FUNC_DEFINE( manager, crude_animation_player );
...
CRUDE_PARSE_JSON_TO_COMPONENT_FUNC_IMPLEMENTATION( crude_camera )
{
  ...
  component->fov_radians = CRUDE_CAST( float32, cJSON_GetNumberValue( cJSON_GetObjectItemCaseSensitive( component_json, "fov_radians" ) ) );
  component->near_z = CRUDE_CAST( float32, cJSON_GetNumberValue( cJSON_GetObjectItemCaseSensitive( component_json, "near_z" ) ) );
  component->far_z = CRUDE_CAST( float32, cJSON_GetNumberValue( cJSON_GetObjectItemCaseSensitive( component_json, "far_z" ) ) );
  component->aspect_ratio = CRUDE_CAST( float32, cJSON_GetNumberValue( cJSON_GetObjectItemCaseSensitive( component_json, "aspect_ratio" ) ) );
  return true;
}
...
CRUDE_PARSE_JSON_TO_COMPONENT_FUNC_IMPLEMENTATION( crude_gltf )
{
  ...
  crude_string_view                                        gltf_relative_filepath;

  gltf_relative_filepath = crude_string_view_make( cJSON_GetStringValue( cJSON_GetObjectItemCaseSensitive( component_json, "path" ) ) );
  ...
  *component = CRUDE_COMPOUNT_EMPTY( crude_gltf );
  ...
  component->debug_only = cJSON_HasObjectItem( component_json, "debug_only" ) ? cJSON_GetNumberValue( cJSON_GetObjectItemCaseSensitive( component_json, "debug_only" ) ) : false;
...
CRUDE_PARSE_COMPONENT_TO_JSON_FUNC_IMPLEMENTATION( crude_gltf )
{
  ...
  model_renderer_resources = crude_gfx_model_renderer_resources_manager_access( manager->model_renderer_resources_manager, component->model_renderer_resources_instance.model_renderer_resources_handle );
  ...
  if ( component->debug_only )
  {
    cJSON_AddItemToObject( gltf_json, "debug_only", cJSON_CreateBool( component->debug_only ) );
  }
...
CRUDE_PARSE_COMPONENT_TO_IMGUI_FUNC_IMPLEMENTATION( crude_gltf )
{
  ...
  CRUDE_IMGUI_OPTION( "Relative Filepath", {
    ...
    if ( ImGui::BeginDragDropTarget( ) )
    {
      ...
      crude_string_view                                    replace_relative_filepath;
    
      im_payload = ImGui::AcceptDragDropPayload( "crude_content_browser_file" );
      if ( im_payload )
      {
        replace_relative_filepath = crude_string_view_make( CRUDE_CAST( char const*, im_payload->Data ) );
        if ( crude_string_raw_substr( replace_relative_filepath.data, ".gltf" ) )
        {
          crude_gfx_model_renderer_resources              *model_renderer_resources;
  ...
  CRUDE_IMGUI_OPTION( "Debug", {
    ImGui::Checkbox( "##Debug", &component->debug_only );
  } );
...
CRUDE_PARSE_COMPONENT_TO_IMGUI_FUNC_IMPLEMENTATION( crude_node_external )
{
  ...
  if ( ImGui::BeginDragDropTarget( ) )
  {
    ...
    {
      replace_relative_filepath = crude_string_view_make( CRUDE_CAST( char const*, im_payload->Data ) );
      if ( crude_string_raw_substr( replace_relative_filepath.data, ".crude_node" ) )
      {
        crude_node_external                                node_external;
        crude_entity                                       new_external_node;
        crude_entity                                       parent;
        char                                               new_name[ CRUDE_NODE_RELATIVE_FILEPATH_LENGTH_MAX ];

        node_external = crude_node_external_empty( );
        node_external.type = CRUDE_NODE_EXTERNAL_TYPE_REFERENCE;
        crude_string_raw_copy( node_external.node_relative_filepath, replace_relative_filepath.data, sizeof( node_external.node_relative_filepath ) );
        
        parent = crude_entity_get_parent( world, node );

        crude_string_raw_copy( new_name, crude_entity_get_name( world, node ), sizeof( new_name ) );

        crude_entity_destroy_hierarchy( world, node );

        new_external_node = crude_node_manager_create_node( manager, replace_relative_filepath, world );
        crude_entity_set_parent( world, new_external_node, parent );
        crude_entity_set_name( world, new_external_node, new_name );
        CRUDE_ENTITY_SET_COMPONENT( world, new_external_node, crude_node_external, { node_external } );
      }
    }
    ImGui::EndDragDropTarget();
  }
...
CRUDE_PARSE_JSON_TO_COMPONENT_FUNC_IMPLEMENTATION( crude_animation_player )
{
  crude_memory_set( component, 0, sizeof( crude_animation_player ) );
  return true;
}

CRUDE_PARSE_COMPONENT_TO_JSON_FUNC_IMPLEMENTATION( crude_animation_player )
{
  cJSON                                                   *component_json;

  component_json = cJSON_CreateObject( );
  cJSON_AddItemToObject( component_json, "type", cJSON_CreateString( CRUDE_COMPONENT_STRING( crude_animation_player ) ) );
  
  return component_json;
}

CRUDE_PARSE_COMPONENT_TO_IMGUI_FUNC_IMPLEMENTATION( crude_animation_player )
{
  CRUDE_IMGUI_START_OPTIONS;
}
...
CRUDE_PARSE_JSON_TO_COMPONENT_FUNC_IMPLEMENTATION( crude_terrain )
{
  ...
  crude_string_view                                        height_texture_relative_filepath;
  ...
  height_texture_relative_filepath = crude_string_view_make( cJSON_GetStringValue( cJSON_GetObjectItemCaseSensitive( component_json, "height_texture" ) ) );
  if ( CRUDE_STRING_VIEW_CMP( height_texture_relative_filepath, "none" ) != 0 )
  {
...
CRUDE_PARSE_COMPONENT_TO_IMGUI_FUNC_IMPLEMENTATION( crude_terrain )
{
  ...
  CRUDE_IMGUI_OPTION( "Height Texture", {
    ...
    if ( ImGui::BeginDragDropTarget( ) )
    {
      ...
      {
        replace_relative_filepath = crude_string_view_make( CRUDE_CAST( char const*, im_payload->Data ) );
        if ( crude_string_raw_substr( replace_relative_filepath.data, ".png" ) )
        {
...
CRUDE_PARSE_JSON_TO_COMPONENT_FUNC_IMPLEMENTATION( crude_world_environment )
{
  ...
  crude_parse_json_to_float3( &component->background_color, cJSON_GetObjectItemCaseSensitive( component_json, "background_color" ) );
  component->background_intencity = cJSON_GetNumberValue( cJSON_GetObjectItemCaseSensitive( component_json, "background_intencity" ) );
  
  crude_parse_json_to_float3( &component->ambient_color, cJSON_GetObjectItemCaseSensitive( component_json, "ambient_color" ) );
  component->ambient_intencity = cJSON_GetNumberValue( cJSON_GetObjectItemCaseSensitive( component_json, "ambient_intencity" ) );
  
  component->ssr.type = CRUDE_CAST( crude_world_environment_ssr_type, CRUDE_MIN( CRUDE_WORLD_ENVIRONMENT_SSR_TYPE_COUNT - 1, cJSON_GetNumberValue( cJSON_GetObjectItemCaseSensitive( component_json, "ssr.type" ) ) ) );
  component->ssr.max_steps = cJSON_GetNumberValue( cJSON_GetObjectItemCaseSensitive( component_json, "ssr.max_steps" ) );
  component->ssr.fade_start = cJSON_GetNumberValue( cJSON_GetObjectItemCaseSensitive( component_json, "ssr.fade_start" ) );
  component->ssr.fade_end = cJSON_GetNumberValue( cJSON_GetObjectItemCaseSensitive( component_json, "ssr.fade_end" ) );
...
CRUDE_PARSE_COMPONENT_TO_JSON_FUNC_IMPLEMENTATION( crude_world_environment )
{
  ...
  cJSON_AddItemToObject( world_environment_json, "background_color", cJSON_CreateFloatArray( &component->background_color.x, 3 ) );
  cJSON_AddItemToObject( world_environment_json, "background_intencity", cJSON_CreateNumber( component->background_intencity ) );
  
  cJSON_AddItemToObject( world_environment_json, "ambient_color", cJSON_CreateFloatArray( &component->ambient_color.x, 3 ) );
  cJSON_AddItemToObject( world_environment_json, "ambient_intencity", cJSON_CreateNumber( component->ambient_intencity ) );

  cJSON_AddItemToObject( world_environment_json, "ssr.type", cJSON_CreateNumber( component->ssr.type ) );
  cJSON_AddItemToObject( world_environment_json, "ssr.max_steps", cJSON_CreateNumber( component->ssr.max_steps ) );
  cJSON_AddItemToObject( world_environment_json, "ssr.fade_start", cJSON_CreateNumber( component->ssr.fade_start ) );
  cJSON_AddItemToObject( world_environment_json, "ssr.fade_end", cJSON_CreateNumber( component->ssr.fade_end ) );
...
CRUDE_PARSE_COMPONENT_TO_IMGUI_FUNC_IMPLEMENTATION( crude_world_environment )
{
  ...
  CRUDE_IMGUI_OPTION( "Ambient Color", {
    ImGui::ColorEdit3( "##Ambient Color", &component->ambient_color.x );
  } );

  CRUDE_IMGUI_OPTION( "Ambient Intencity", {
    ImGui::DragFloat( "##Ambient Intencity", &component->ambient_intencity );
  } );
  

  CRUDE_IMGUI_OPTION( "SSR Type", {
    if ( ImGui::BeginCombo( "##SSR Type", crude_world_environment_ssr_type_to_str( component->ssr.type ) ) )
    {
      for ( uint32 i = 0; i < CRUDE_WORLD_ENVIRONMENT_SSR_TYPE_COUNT; i++ )
      {
        crude_world_environment_ssr_type                   type;
        bool                                               is_selected;

        type = CRUDE_CAST( crude_world_environment_ssr_type, i );
        is_selected = ( component->ssr.type == type );

        if ( ImGui::Selectable( crude_world_environment_ssr_type_to_str( type ), is_selected ) )
        {
          component->ssr.type = type;
        }
  ...
  CRUDE_IMGUI_OPTION( "SSR Max Steps", {
    ImGui::DragFloat( "##SSR Max Steps", &component->ssr.max_steps, 1.f, 0.f );
  } );
  
  CRUDE_IMGUI_OPTION( "SSR Fade Start", {
    ImGui::DragFloat( "##SSR Fade Start", &component->ssr.fade_start, 0.05f, 0.f, 1.f );
  } );
  
  CRUDE_IMGUI_OPTION( "SSR Fade End", {
    ImGui::DragFloat( "##SSR Fade End", &component->ssr.fade_end, 0.05f, 0.f, 1.f );
  } );
}
