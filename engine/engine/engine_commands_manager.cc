...
)
{
  ...
  crude_string_raw_copy( command.load_node.relative_filepath, relative_filepath.data, sizeof( command.load_node.relative_filepath ) );
  CRUDE_ARRAY_PUSH( manager->commands_queue, command );
}
...
//    }
    case CRUDE_ENGINE_COMMANDS_MANAGER_QUEUE_COMMAND_TYPE_LOAD_NODE:
    {
      ...
      {
        crude_string_raw_copy( manager->engine->main_node_relative_filepath, manager->commands_queue[ i ].load_node.relative_filepath, sizeof( manager->engine->main_node_relative_filepath ) );
      }

      manager->engine->main_node = crude_node_manager_create_node( &manager->engine->node_manager, crude_string_view_make( manager->commands_queue[ i ].load_node.relative_filepath ), manager->engine->world );
