...
)
{
  ...
  resource_pool->memory = CRUDE_CAST( uint8*, CRUDE_ALLOCATE( allocator, allocation_size ) );
  memset( resource_pool->memory, 0, allocation_size );
  
  resource_pool->free_indices = CRUDE_CAST( uint32*, resource_pool->memory + pool_size * resource_size );
