...
(
  _In_ uint8                                              *h,
  _In_ crude_string_view const                             key,
  _In_ size_t                                              elemsize
);
...
#define CRUDE_HASHMAPSTR_HEADER( h )  ( CRUDE_CAST( crude_hashmapstr_header*, h ) - 1 )
...
#define CRUDE_HASHMAPSTR_INITIALIZE( h, l ) ( ( h ) = NULL, ( h ) = CRUDE_CAST( CRUDE_TYPE( h ), crude_hashmapstr_growf( NULL, sizeof*( h ), CRUDE_CORE_HASHMAP_INITIAL_CAPACITY, ( l ) ) ) )
#define CRUDE_HASHMAPSTR_INITIALIZE_WITH_CAPACITY( h, c, l ) ( ( h ) = NULL, ( h ) = CRUDE_CAST( CRUDE_TYPE( h ), crude_hashmapstr_growf( NULL, sizeof*( h ), c, ( l ) ) ) )
#define CRUDE_HASHMAPSTR_DEINITIALIZE( h ) ( CRUDE_DEALLOCATE( CRUDE_HASHMAPSTR_ALLOCATOR( h ), CRUDE_HASHMAPSTR_HEADER( h ) ) )
#define CRUDE_HASHMAPSTR_GET_INDEX( h, k ) ( crude_hashmapstr_get_index( CRUDE_CAST( uint8*, h ), k, sizeof*( h ) ) )
...
#define CRUDE_HASHMAPSTR_SET( h, k, v ) ( ( h ) = CRUDE_CAST( CRUDE_TYPE( h ), crude_hashmapstr_set_index( CRUDE_CAST( uint8*, h ), k, sizeof*( h ) ) ), ( h )[ CRUDE_HASHMAPSTR_TEMP( h ) ].value = v )
