...
{
  return crude_hash_bytes( CRUDE_CAST( uint8 const*, str.data ), str.length, seed );
}
...
)
{
  ...
  nh = CRUDE_CAST( uint8*, CRUDE_ALLOCATE( allocator, sizeof( crude_hashmapstr_header ) + elemsize * cap ) );
  ...
  if ( h )
  {
    ...
    {
      crude_hashmapstr *backet = CRUDE_CAST( crude_hashmapstr*, h + i * elemsize );
...
)
{
  ...
  {
    nh = CRUDE_CAST( uint8*, crude_hashmapstr_growf( h, elemsize, CRUDE_HASHMAPSTR_CAPACITY( h ) * 2, CRUDE_HASHMAPSTR_ALLOCATOR( h ) ) );
  }
  else
  {
    nh = h;
  ...
  backet = CRUDE_CAST( crude_hashmapstr*, nh + elemsize * index );
