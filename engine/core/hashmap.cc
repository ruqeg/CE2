...
)
{
  ...
  nh = CRUDE_CAST( uint8*, CRUDE_ALLOCATE( allocator, sizeof( crude_hashmap_header ) + elemsize * cap ) );
  ...
  if ( h )
  {
    ...
    {
      hash_backet *backet = CRUDE_CAST( hash_backet*, h + i * elemsize );
...
)
{
  ...
  backet = CRUDE_CAST( hash_backet*, h + elemsize * index );
...
)
{
  ...
  {
    nh = CRUDE_CAST( uint8*, crude_hashmap_growf( h, elemsize, CRUDE_HASHMAP_CAPACITY( h ) * 2, CRUDE_HASHMAP_ALLOCATOR( h ) ) );
  }
  else
  {
    nh = h;
  ...
  backet = CRUDE_CAST( hash_backet*, nh + elemsize * index );
