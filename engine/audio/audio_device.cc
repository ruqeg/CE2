...
{
  ma_sound                                                *lma_sound;
  crude_string_view                                        absolute_filepath;
  ...
  crude_string_buffer_initialize( &absolute_filepath_string_buffer, audio->environment->resources_absolute_directory.length + crude_string_raw_calculate_length( creation->relative_filepath ) + 1, crude_heap_allocator_pack( audio->allocator ) ); 
  absolute_filepath = crude_string_buffer_append_use_f( &absolute_filepath_string_buffer, "%s%s", audio->environment->resources_absolute_directory.data, creation->relative_filepath );
  result = ma_sound_init_from_file( &audio->lma_engine, absolute_filepath.data, sounnd_flags, NULL, creation->async_loading ? &audio->lma_fence : NULL, lma_sound );
