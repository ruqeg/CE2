...
CRUDE_PARSE_JSON_TO_COMPONENT_FUNC_IMPLEMENTATION( crude_audio_player )
{
  ...
  crude_string_raw_copy( component->relative_filepath, cJSON_GetStringValue( cJSON_GetObjectItemCaseSensitive( component_json, "path" ) ), sizeof( component->relative_filepath ) );
  ...
  CRUDE_IMGUI_OPTION( "Relative Filepath", {
    ...
    if ( ImGui::BeginDragDropTarget( ) )
    {
      ...
      if ( im_payload )
      {
        ...
        {
          modified = true;
          crude_string_raw_copy( component->relative_filepath, replace_relative_filepath, sizeof( component->relative_filepath ) );
        }
      }
...
)
{
  ...
  for ( uint32 i = 0; i < it->count; ++i )
  {
    ...
    if ( audio_player->relative_filepath[ 0 ] )
    {
      ...
      crude_string_raw_copy( sound_creation.relative_filepath, audio_player->relative_filepath, sizeof( sound_creation.relative_filepath ) );
