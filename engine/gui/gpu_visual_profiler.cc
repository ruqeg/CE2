...
)
{
  ...
  profiler->max_visible_depth = 3;
  profiler->max_queries_per_frame = gpu->gpu_time_queries_per_frame;
...
)
{
  ...
  for ( uint32 i = 0; i < active_timestamps; ++i )
  {
    ...
    if ( hash_color_index == -1 )
    {
      ...
      CRUDE_HASHMAPSTR_SET( profiler->name_to_color_index, timestamp->name, color_index );
    }
    else
    {
...
)
{
  ...
  
  {
    ...
    device_name = crude_gfx_rhi_get_device_name( &profiler->gpu->rhi_device );

    ImGui::Text( "GPU used: %s", device_name.data );
