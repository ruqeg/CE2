...
)
{
  ...
  crude_string_raw_copy( node->state, data, size );
...
)
{
  ...
  if ( data != NULL )
  {
    length = crude_string_raw_copy_unknow_length( data, node->state, sizeof( node->state ) );
  }
  return length;
}
...
)
{
  ...
  for ( uint64 node_index = 0; node_index < CRUDE_ARRAY_LENGTH( blueprint->nodes ); ++node_index )
  {
    ...
    if ( !node->saved_state[ 0 ] )
    {
      ...
      {
        crude_string_raw_copy( node->saved_state, node->state, sizeof( node->saved_state ) );
      }
    ...
    if ( node->saved_state[ 0 ] )
    {
      ...
      {
        crude_string_raw_copy( node->state, node->saved_state, sizeof( node->state ) );
