...
(
  ...
  _Inout_ b3Vec3                                         **b3_vertices,
  _Inout_ int32                                          **b3_indices,
...
)
{
  ...
  CRUDE_HASHMAPSTR_SET( manager->mesh_shape_relative_filepath_to_hadle, CRUDE_COMPOUNT( crude_string_link, { shape_mesh_container->relative_filepath } ), shape_mesh_handle );
#if CRUDE_DEVELOP
  ...
  for ( uint32 i = 0u; i < gltf_nodes_count; ++i )
  {
    ...
    if ( gltf_nodes[ i ]->mesh )
    {
      for ( uint32 pi = 0; pi < gltf_nodes[ i ]->mesh->primitives_count; ++pi )
      {
        ...
        vertices_offset = CRUDE_ARRAY_LENGTH( *b3_vertices );
        ...
        for ( uint32 i = 0; i < vertices_count; ++i )
        {
          ...
          b3_position.x = XMVectorGetX( p );
          b3_position.y = XMVectorGetY( p );
          b3_position.z = XMVectorGetZ( p );

          CRUDE_ARRAY_PUSH( *b3_vertices, b3_position );
        }

        for ( uint32 i = 0; i < indices_count; ++i )
        {
          CRUDE_ARRAY_PUSH( *b3_indices, vertices_offset + indices[ i ] );
        }
      }
    }

    crude_physics_shapes_manager_gltf_load_nodes_( manager, gltf, gltf_nodes[ i ]->children, gltf_nodes[ i ]->children_count, b3_vertices, b3_indices, node_to_parent );
  }
}
...
)
{
  ...
  CRUDE_ARRAY_INITIALIZE_WITH_LENGTH( b3_vertices, 0, crude_heap_allocator_pack( manager->allocator ) );
  CRUDE_ARRAY_INITIALIZE_WITH_LENGTH( b3_indices, 0, crude_heap_allocator_pack( manager->allocator ) );
  ...
  crude_string_copy( mesh_shape_container->relative_filepath, gltf_relative_filepath, sizeof( mesh_shape_container->relative_filepath ) );

  for ( uint32 i = 0; i < gltf->scenes_count; ++i )
  {
    crude_physics_shapes_manager_gltf_load_nodes_( manager, gltf, gltf->scene[ i ].nodes, gltf->scene[ i ].nodes_count, &b3_vertices, &b3_indices, XMMatrixIdentity( ) );
  }
  
	b3_mesh_def = CRUDE_COMPOUNT_EMPTY( b3MeshDef );
	b3_mesh_def.vertices = b3_vertices;
	b3_mesh_def.vertexCount = CRUDE_ARRAY_LENGTH( b3_vertices );
	b3_mesh_def.indices = b3_indices;
	b3_mesh_def.triangleCount = CRUDE_ARRAY_LENGTH( b3_indices ) / 3;
...
cleanup:
  CRUDE_ARRAY_DEINITIALIZE( b3_vertices );
  CRUDE_ARRAY_DEINITIALIZE( b3_indices );
